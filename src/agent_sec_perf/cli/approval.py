...
⚠️ **已知缺口（2026-09-19 领导裁决；本模块如实登记，不掩盖）**：本类**必须**先 ``emit`` 再返回
（``A4``），因此"审计写入失败"会**在这里抛出**；而 ``harness/loop.py`` 的 ``R3`` 会把 gate 的
任何异常收敛成 ``ERROR(INTERNAL)`` + ``TASK_FINISHED(FAILED)``（固定文案"人工确认通路故障"）
⇒ **审计失败与 gate 自身故障在事件上不可分**，且该文案在审计失败时会**表述失真**。
退出码仍正确（``cli`` 的 ``_AuditRecorder`` 旁路标记保证 ``4``）。完整边界与下轮修法见
``cli/app.py`` 的 ``_AuditRecorder`` docstring。本类**不捕获** ``emit`` 的异常——证据面故障
不得被吞（``audit.md`` §2.4）。
