...
四条签名的 `cpu` 字段**完全相同**（`AMD EPYC 9K65 192-Core Processor`）：
- prefill 散布 **≈14%**（121.68 → 106.79），且**后三条聚集在 106.8~109.5**（首条 121.68 偏高）；
- gen 散布 **≈32%**（25.70 → 33.84），其中 **33.84 是明显的离群点**
  （其余三条为 25.70 / 26.56 / 27.24），**成因未定**。
