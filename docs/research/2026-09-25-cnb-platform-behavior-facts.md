...
| 13 | **⭐ 配置（含 `crontab`）从「事件发生在哪条分支」的**自身树**里读** ⇒ **停用一条分支上的定时任务，必须动那条分支自己**；在 `develop` 上删掉 `crontab` **不会**影响别的分支仍在声明的定时任务。**实测教训**：本仓库 `bench/nightly`（tip 停在 2026-09-17）自身仍声明两条 `crontab`（`0 4 * * 2-6,0` 夜轮 + `0 4 * * 1` 深跑）⇒ ADR-0023 在 `develop` 上"停用全部自动跑测"之后，**夜轮其实一直在按点运行**（近 100 条构建里有 4 次 `event: "crontab: …"`、`sourceRef: bench/nightly`；最近一次 `sn=cnb-2tf-1k3ag1u6t` 就是当天 04:00 那轮，数据分支的 `is_cron=true` 亦为佐证） | ✅ 2026-09-25：`git show bench/nightly:.cnb.yml` 的顶层键为 `"**"` + `bench/nightly:`（内含两处 `crontab`）+ 构建列表。⚠️ 与第 9 条（数据分支的门禁红灯）**同一族**：都源于"改了 A 分支，但生效的是 B 分支自己的配置" |
...
| `2026-09-25-devkey-check`（现役） | `cnb-1oj-1k3bbs9na` | S × 3 | **106.79** | 25.70 | ADR-0028 落地后在 `develop` 上触发；**独立 label ⇒ 未被覆盖** |
| `2026-09-25-datafix-check`（现役） | `cnb-aac-1k3be85pm` | S × 3 | **120.38** | 35.41 | 数据分支清理修复后的验证轮；**独立 label ⇒ 未被覆盖** |

五条签名的 `cpu` 字段**完全相同**（`AMD EPYC 9K65 192-Core Processor`）：
- prefill 散布 **≈14%**（106.79 → 121.68），并呈现**两个簇**：{121.68, 120.38} 与 {109.50, 107.58, 106.79}；
- gen 散布 **≈38%**（25.70 → 35.41），也有两个簇：{25.70, 26.56, 27.24} 与 {33.84, 35.41}；
- ⚠️ 两指标**不同步**（例如 121.68/27.24 与 120.38/35.41 prefill 几乎相同而 gen 差 30%）
  ⇒ **不能**用"某台机器快/慢"一句话解释，成因仍**未定**（宿主差异 / 频率 / 负载 / 运行次序都可能）。
...
| ~~V7~~ | ~~同时存在的容器数上限（跨仓库、按人/组织）是多少；撞上时是排队还是失败~~ | ✅ **已决定不验证**（2026-09-25 所有者裁决：探针要同时占 ≥5 个环境，与其它仓库争抢同一份额度、收益不足）⇒ §9.1 的 #5/#6 **保持原状态**（"未实测" / "未验证"），**不得**据此把"≈6"写成机制 |
...
| 6 | 撞上限后的表现（第 N+1 个是**排队 pending**、**直接失败**、还是报错文案） | ❌ 未验证 | 无任何来源提及；见 §7 V7 与 §9.3（**已决定不验证**） |
...
### 9.3 探针配方（⚠️ **已决定不执行** —— 2026-09-25 所有者裁决；配方**归档保留**，不要重提）

> **为什么不执行**：它要**同时**拉起 ≥5 个环境（已实测下界 = 4）才可能撞上限，而并发额度是
> **跨仓库、按人/组织共享**的（§9.1 #3）；本仓库此刻仍有其它项目在跑 ⇒ 探针会挤占它们。
> 收益（一个官方未公布的运维数字）与代价不成比例 ⇒ **放弃**。
> ⚠️ 因此 §9.1 的 #5（≈6，**未实测**）与 #6（撞上限表现，**未验证**）**长期保持未验证状态**：
> 任何文档**不得**把"6 个"写成机制或保证。若将来真的撞上限，按 §9.1 #6 的缺口**现场记录**即可，
> 不必为它专门开探针。
...
> 上面的配方**仅作归档**（"退役并归档，不是丢弃"，同 `.cnb.yml` 跑测段的口径）：
> 若将来确实需要这个数字（例如真要开分片跑测），先用 `cnb workspace list-workspaces --status running`
> 看当前占用，再决定是否值得占满额度。<br>
> **2026-09-25 决定**：本探针**不做**（见本节开头）；此外当时机到来时，**任何文档仍不得把
> "6 个"写成机制或保证**。
