...
| [reproducibility-and-gates.md](reproducibility-and-gates.md) | 复现性与门禁：版本分叉、证据不可变形、构建期静默失败、**基础镜像的时间漂移**（同一份配置在不同镜像/不同时间语义不同），以及**被平台杀掉的构建**（"无输出超时"会把网络卡顿伪装成 Dockerfile 写错）与 **`curl … \| sh` 的失败吞噬**、**`--no-cache` 使"层缓存省时间"不成立** | devlog 0006 / 0007 / 0012 / 0020 |
