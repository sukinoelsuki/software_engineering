...
## 7. 关联处置：本地钩子层与 CI 密钥扫描（2026-09-18，已执行）

> 本节是**同一类问题的第二个实例**——上一节（§0~§5.1）是"配置写了但没生效"，
> 本节是"**声称的保护根本没在运行**"（一致性报告 A-11 + 新增 A-15）。
> 之所以放在本文：本文的定位是"'门禁配置是否真的生效'这一类问题的落点"。

**事故**（详见 [`devlog 0014`](../devlog/0014-2026-09-18-威胁模型与安全修复.md) §3/§5）：

- `Makefile` 的 `setup` 目标在 `CI=true` 时**跳过钩子安装**，而云开发工作区恰好带着 `CI=true`
  ⇒ `.git/hooks/` **长期为空**，`.pre-commit-config.yaml` 声明的 `detect-private-key`
  与 commit-msg 校验**从未运行**；
- 当时"密钥扫描"**既不在 CI、也不在本地**运行（A-11 的实质），而文档两处都声称它在；
- **没有任何检查会因"钩子不存在"而失败** ⇒ 该状态长期无人察觉（A-15 的失效模式）。

**所有者裁决（2026-09-18）**：按推荐**两条都做**（① CI 增加密钥扫描；② 把"钩子存在"做成可执行检查），
并把 `Makefile` 的 `CI=true ⇒ 跳过装钩` 改为**显式开关**。

**处置后的三处防线（每一处都可实测）**：

| 位置 | 载体 | 怎么实测它在运行 | 缺失时 |
| --- | --- | --- | --- |
| 提交时（本地） | `.git/hooks/pre-commit`、`.git/hooks/commit-msg`（pre-commit 生成） | `make hooks-check`（`scripts/check-local-hooks.sh`） | 非零退出 + 给出 `make setup` |
| 本地全量门禁 | `make check` → `security-secrets` | `make check` 的**第一步**就是 `hooks-check` | 门禁红（不是"跑完才知道"） |
| CI | `.cnb.yml` 的具名 `secret-scan` stage（`make security-secrets`） | 流水线上**可见的 stage** | 构建红 |

**新的不变式（硬性）**：

1. **钩子安装不得由环境变量推断**：一律用显式开关 `LOCAL_HOOKS`
   （默认 `1` = 安装**并断言**；`0` = 不安装、不断言，**仅** CI 流水线使用）。
   理由见 §下"为什么不能用 `$CI`"。
2. **`make check` 的第一条断言是"本地钩子层真的在"**（`hooks-check`）⇒ 本地防线缺失时立即失败。
3. **密钥扫描只有一份实现**：`pre-commit` 的 `detect-private-key`。本地钩子、`make check`
   与 CI 的 stage 调的都是它，不另写一套正则/关键词扫描（避免两套口径，即 §1 的老问题）。
4. **每个门禁流水线都必须有具名 `secret-scan` stage**，由
   `tests/unit/test_cnb_config.py` 的**计数断言**（门禁流水线数 == 具名阶段数）保证。
5. **检查本身必须非恒过**：`tests/unit/test_local_gates.py` 用临时 git 仓库复现
   "缺失 / 不可执行 / 被顶替 / 指向别的配置 / 钩子装串"五种状态，逐一断言报红。

**为什么不能用 `$CI` 当判据**：`CI` 的**存在性**对"云开发工作区"与"CI 流水线"是同一个值，
语义却相反。2026-09-18 实测：本工作区 `CI` **未设置**，而更早的工作区 `CI=true` 只是云环境。
拿它当判据会得到"看起来装了、实际没装"的防线——正是 A-15 那类失效模式
（同一裁决里，T-08 残余项的"按 `CI` 运行时拒绝 `root` 隔离"也被否掉，理由相同）。

**已知边界（如实标注，勿当已闭环）**：

| # | 边界 | 说明 |
| --- | --- | --- |
| 1 | `pre-commit run --all-files` 只覆盖**已跟踪**文件（走 `git ls-files`） | 新写但**未 `git add`** 的文件不在 `make security-secrets` 范围内；提交时的钩子看**暂存区**，能覆盖它；CI 里所有文件都已跟踪，不受影响 |
| 2 | 首次运行需网络拉取 hook 仓库 | CI 有网；无网时会红灯，属 fail-secure（宁可红，不可静默放过） |
| 3 | "钩子是 pre-commit 生成的"依赖其模板标记 | 依据：`.git/hooks/pre-commit` 第 2 行的 `# File generated by pre-commit`。上游改模板会误报红 ⇒ 需同步脚本匹配规则（可接受：误报红优于漏报绿） |
| 4 | 未验证 CI 上的实际运行 | 本轮**未执行**一次真实流水线（本地无 CNB runner）；`secret-scan` stage 的语法与 `make security-secrets` 已在本地跑通，但"CI 里真的执行了"待下次 push 后核对 |

**验证方式（可执行；下列均已实跑）**：

```text
$ make check                                   # → 128 passed；含 security-secrets Passed
$ bash scripts/check-local-hooks.sh            # → rc=0（pre-commit / commit-msg 均 OK）

# 变异探针 A：移走 pre-commit 钩子 ⇒ 断言必须报红（跑完立即还原并复核）
$ mv .git/hooks/pre-commit .git/hooks/pre-commit.disabled
$ make hooks-check                             # → rc=2，打印 "pre-commit：钩子不存在" + 修复命令
$ mv .git/hooks/pre-commit.disabled .git/hooks/pre-commit

# 变异探针 C：把 Makefile 默认隔离模式改成 root ⇒ 静态守卫必须报红（跑完立即还原）
#   → 2 failed（test_makefile_default_isolation_is_user / test_neither_makefile_nor_ci_selects_root_isolation）

# 变异探针 D：删掉某个门禁流水线的 secret-scan 阶段 ⇒ 计数断言必须报红（跑完立即还原）
#   → 1 failed（具名密钥扫描阶段数（1）与门禁流水线数（2）不一致）
```
...
- **2026-09-18（关联处置，同一类问题的第二个实例）**：**新增 §7**——本地钩子层长期缺失
  （A-11 / 新增 A-15）的处置：`LOCAL_HOOKS` 显式开关、`make hooks-check`
  （`scripts/check-local-hooks.sh`）、`make security-secrets` 与 CI 具名 `secret-scan` stage、
