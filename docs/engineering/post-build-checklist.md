...
> ⚠️ **上表里的版本号（`code-server 1.137.0`、`codebuddy 2.151.0`）是 2026-09-15 的实测值，
> 不是钉住的版本**：`code-server` 由官方 `install.sh` 取最新版 ⇒ 该数字会漂移
> （2026-09-21 重建后实测已是 `1.138.0`）。核验"镜像生效"请用 `code-server --version` 实测，
> **不要照抄本表**。
>
