...
| `bench/data` | `bench/nightly` / `test/*`（**跑测脚本**推入） | **不合入** | 快进推送，**禁止 force** | **长驻**（例外，见下） |
...
- **它们不合入 `develop`，也不得删除**——这与 §2 表中其他分支的"短期 + 合回"规则**相反**，
  是刻意的例外，不是遗漏；
- `bench/data`（机器产出：JSON / 报告 / 日志 / 模型产物）**只由跑测脚本写入**
  （2026-09-24 前是"只由 CI 写"；CI 已不再跑测，见
  [ADR-0023](../adr/0023-ci-downgrade-to-manual-trigger.md)），
  以快进推送更新、**禁止 force**，来源分支白名单为 `bench/nightly` / `test/*`；
  **人手不得直接提交**；
- `bench/nightly`（基准代码）**不回写代码到 `develop`**；要进产品线的改动另开普通提交；
  **2026-09-24 起它不再被任何流水线读取**，因此**不需要再同步**——保留它只是为了
  "不删除已推送的历史"（D 类禁止删远端分支）；
- **测试数据不得进任何代码分支**：`test/<slug>`、`develop`、`main` 都不许出现
  `.bench-data` 的产出；大文件走制品库。
...
> "CI 推数据 → 又触发一轮测试"的死循环。
>
> ⚠️ **其中两条已随 ADR-0023 失效**（"`crontab` 只支持单分支"与"定时任务取分支 HEAD"
> 都以"存在定时任务"为前提，而定时任务已整体移除）。ADR-0014 按"只增不改"**保留原文**；
> 现行的**分支与数据纪律**以本节与
...
| **`test/*` 的"零提交"** | ✅ **必须出现（一行）** | 摘要的默认叙述是"开分支 → 提交 → 合回 `develop`"；而 `test/<slug>` **只借环境、不许提交、不用合回**。省略会让默认推导落到"把跑测改动提交到这条分支上"，而它用完即弃 ⇒ 改动**下次会话不存在**（同 `develop` 直推那一节的机制） |
...
| **多会话并发的提交 / 推送范围纪律**（§6） | ✅ **必须出现（一小节）** | 摘要现在只写"本地 `add` / `commit` 可在当前任务范围内进行"；**同一工作区跑多个会话时**，按此默认推导会落到"**直接 `git add -A` 再提交**"这一**相反且有害**的动作（把他人的未提交改动一并暂存并可能推出远端）。故 **M-1 / M-2 / M-5 必须进摘要**（`-o` 点名提交、禁止整仓暂存、推送前核对归属）；其余细则（M-3~M-4、M-6~M-9、推送被拒的处置）可省略 |
...
| **E** | `bench/data` | **只由跑测脚本写**（`make bench`），**人手不得直接提交**；禁止把测试数据推到任何代码分支 |
...
| E · `bench/data` | ❌ 禁止 force，只能前进 | — | ❌ 坏点永久污染"同签名序列" | 不适用 ⇒ 只由跑测脚本写（`make bench`） |
...
2. **B ~ F 类**：**仍须事先批准 / 确认**，**禁止**把 A 类的"事后报告"外推——
   ...
   `bench/data` 只由跑测脚本写（`make bench`）；新增依赖/改门禁强度/改规则本身事先确认。
3. 代理**不得**自行执行 `git reset --hard`、改写已推送历史、强推、删除远端分支等破坏性操作。
4. **同一工作区跑多个会话时**，提交与推送还需遵守 **§6「多会话并发纪律」**：
   **A 类的"事后报告"在多会话下有附加要求**（报告须覆盖"**实际推送的全部提交**"，
   并对非本会话的提交单独标注）——见 §6.4。这不是新增授权，而是让 §4 的
   "**报告与仓库不一致即视为未报告**"在多会话下仍然成立。
...
## 6. 多会话并发纪律（同一工作区、多个会话）

> **靶子**：**同一工作区里同时跑着多个会话（多个对话）**时的**提交与推送范围**。
> 与 [`agent-teams.md` §6](agent-teams.md)（**一个会话内的多个成员**）**同源不同靶**：
> 那些纪律的成立前提是"**存在一个能看到全部成员、能核对文件域的领导**"
> （`CODEBUDDY.md` §10.2 规则 8 的"派活提示写明产出白名单 + 领导裁决"），
> 而**多会话下没有这个角色**——无信箱、无登记表、不共享上下文 ⇒ **不得直接外推**。
> 决策与论证见 [ADR-0022](../adr/0022-multi-session-concurrency-discipline.md)。

### 6.1 三条全局单例资源（风险都出在这里）

| 资源 | 共享范围 | 后果 |
| --- | --- | --- |
| **git 索引**（`.git/index`） | 全仓库唯一 | 任何会话的 `git add` 都会影响**所有会话**的 `git commit` |
| **工作树** | 全仓库唯一 | 任何会话的写入 / 清理，都会改变**别人看到的**文件与**门禁的读数** |
| **`.git/COMMIT_EDITMSG`** | **单例** | 两次提交不能同时进行，并发提交会互相覆盖提交信息 |

⇒ 核心判断：**"文件不交叉"只保证"内容不互相踩"，保证不了"提交 / 推送不带别人的东西"**。

### 6.2 规则

| # | 规则 | 判据 / 做法 | 为什么 |
| --- | --- | --- | --- |
| **M-1** | **提交范围隔离** | 一律 `git commit -o -m "<msg>" -- <本会话的显式路径列表>`（`--only`） | 只提交点名的路径，共享索引里他人暂存的内容**不会被带走** |
| **M-2** | **禁止整仓暂存** | ❌ `git add -A` / `git add .` / `git add -u` / `git commit -a` / **不带路径且不带 `-o` 的 `git commit`** | 这些命令按**整个工作树**取内容 ⇒ 会把他人**未提交**的改动一并暂存，之后任何一次疏忽即被推上远端 |
| **M-3** | **新文件必须先点名 `add`** | `-o -- <路径>` **对未跟踪路径直接报错**（`pathspec … did not match any file(s) known to git`）⇒ 新文件先 `git add <该文件>`，再 `git commit -o -- <该文件>` | 否则会误以为"点名了就已经隔离好"，实际该文件**根本没进提交** |
| **M-4** | **阻断式核对** | 核对与提交是**两条独立命令**，且核对结果要**用于阻断**：`git diff --cached --name-status` 非空或含非本会话路径 ⇒ `exit 9`；**不是只打印** | "打印了却继续提交"等于没核对（`agent-teams.md` §6 第 4 条，2026-09-18 事故的根因） |
| **M-5** | **推送范围隔离** | `git fetch` → `git log --format='%h %s' @{u}..HEAD` **逐笔确认归属** → 再 `push` | `push` 推的是**整条分支**，会把**同一分支上其他会话尚未推送的提交一并带走**；归属确认见 §6.4 |
| **M-6** | **不与另一会话并发提交** | 发提交前确认没有别的会话正在提交 | `.git/COMMIT_EDITMSG` 是**单例**，被覆盖会让 `commitizen` 报**假红**（2026-09-20 实测） |
| **M-7** | **禁止波及他人的工作树清理** | ❌ `git stash`（**不带路径**）/ `git checkout -- .` / `git restore .` / `git clean -fd`；需要暂存时用 `git stash push -- <自己的路径>` | 这些命令按**整个工作树**作用 ⇒ 会收走 / 抹掉他人**未提交**的改动（那是**唯一无法从 git 恢复**的东西） |
| **M-8** | **门禁读数要判归属** | `make security-secrets` 报 `files were modified by this hook` 时：**无** `- exit code:` ⇒ 判**假红**（先单独复跑）；**有** `- exit code: 1` ⇒ 判**真红**，按安全事件处置 | 细则见 `agent-teams.md` §6「门禁的"红"可能来自共享工作树」；多会话只是**新增了第二个来源**，判据不变 |
| **M-9** | **冲突不自动合并** | 发现同一文件被两个会话改过（`git status` 显示冲突，或某文件出现无法解释的改动）⇒ **停下、报告**，由所有者裁决取舍 | 自动"合并一下就好了"会把**归属问题**变成**内容问题**（`CODEBUDDY.md` §10.2 规则 8 同源） |

### 6.3 推送被拒时的唯一合法路径

远端被推进后 `push` 会**非快进被拒**。此时：

```text
git fetch origin <branch>
git log --oneline --left-right @{u}...HEAD     # 先看清分叉
git pull --rebase origin <branch>              # 或 merge；只作用于「尚未推送」的本地提交
git push
```

- ✅ **允许**对**尚未推送**的本地提交做 `pull --rebase`（`fetch` 后 **rebase 本地未推送的提交，不属于"改写已推送历史"**）。
- ❌ **禁止** `--force` / `--force-with-lease`（**D 类**）；**"被拒"不是使用 force 的理由**。

### 6.4 A 类报告在多会话下的附加要求

§4 的 A 类事后报告含四项，其中两项在多会话下要**加严**：

1. **报告的是"实际推送了什么"，不只是"我提交了什么"**：把 `@{u}..HEAD` 区间内**全部**被推送的提交逐笔列出；
   其中**非本会话**的提交必须**单独标注**「非本会话，门禁状态未由本会话确认」——
   依据仍是 §4 的「**报告与仓库不一致即视为未报告**」。
2. **文件清单须与本会话的写入范围一致**：清单里出现非本会话的路径 ⇒ 说明 M-1~M-4 有一处没做到 ⇒ **先停下报告**。

> **为什么 M-5 不写成"区间含非本会话提交就绝对禁止推送"**：两个会话各自都持有对方未推送的提交时，
> 绝对禁止会让**双方都推不出去**（互相等待）。故 M-5 的默认动作是"**先核对、优先停下报告**"，
> 而**不是**"无条件禁止"；确需继续推时，用上面第 1 条的**报告覆盖**来兜底。

### 6.5 边界（**不得表述为机制**）

- 本节的**唯一前提**是"所有者做会话管理时会避开交叉的写入范围"——这是一条**人员约定**。
  它能降低**内容层**冲突的概率，**不能**保证"提交 / 推送只含本会话内容"；
  后者的护栏是 M-1~M-5 的**逐笔核对**，属**流程纪律**。
- **不可由 CI 强制**：CI 只看到一个 runner、一次工作树，**看不到"是否存在多个会话"**，
  也看不到"报告是否与仓库一致"⇒ 按威胁模型口径记为**部分缓解**（同 §4 的已知边界）。
- **不新增载体**（不建"会话白名单"文件）：见 [ADR-0022](../adr/0022-multi-session-concurrency-discipline.md) §5.2——
  事前声明会与 `docs/devlog/` 的活待办形成**第二份清单**，违反"唯一任务源"；
  改用**提交 / 推送时的机械核对**（M-1~M-5）替代。
- **可选的隔离手段**（**未采纳为默认**）：`GIT_INDEX_FILE` 独立索引、`git worktree` 独立工作树。
  它们能**从根上**隔离，但各有代价（前者需每次 `read-tree` 初始化、`git status` 语义变化、
  与 pre-commit 钩子的交互**未验证**；后者需独立分支，与 `develop` 直推冲突，且新增目录与清理负担）
  ⇒ 记为**待评估**，见 ADR-0022 §10。
