...
| **远端授权的 A~F 分级集合**（§4） | ✅ **必须出现（一表）** | 摘要若不含分级，默认叙述会落到"**任何远端写入都需事先批准**"（或反向的"都不用批准"），两者都是**反向动作**；[ADR-0016](../adr/0016-cnb-platform-integration-and-remote-write-authorization.md) §5.8 第 4 条明确要求 **A 与 B~F 的界线必须出现在摘要里**（2026-09-18 实测：省略即推出反向动作） |
| 完整分支矩阵、合并策略表、PR 与发布全流程、远端授权的**其余细则**（可回退性判据、事后报告格式等） | ❌ 不在摘要 | 完整定义以**本文**与 [ADR-0013](../adr/0013-branch-model-for-solo-dev.md) 为准 |
...
1. **落点**：本次推送所在会话的 `docs/devlog/` **当前篇**新增一条记录，含四项：
   ...
   **落盘职责归记录员**（`docs/devlog/` 是其独占产出域，见 `CODEBUDDY.md` §10.1）
   ⇒ 非记录员角色**只负责向记录员（或团队领导）报告**，**不得**自行写入；
...
1. **A 类**：推送**后**立即按上述格式**向记录员（或团队领导）报告**，由**记录员**
   在 `docs/devlog/` 当前篇落盘（该目录是其独占产出域，见 `CODEBUDDY.md` §10.1）；
...
| `0.2.0` | 端到端主流程可运行（Alpha） | `M2` 出口准则全部达成（`docs/engineering/sdlc.md` §3.3；⚠️ **该节当前为草案、待所有者批准**） |
