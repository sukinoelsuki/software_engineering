...
> - **新增 A-17（2026-09-18 第四轮）**：`git-workflow.md` §5 的 Phase-0 版本语义
...
>   **✅ 已处置（2026-09-19，[ADR-0019](../adr/0019-release-and-version-policy.md)）**：
>   裁决**不在"①改配置 / ②改文档"之间二选一**，而是改掉**产生该问题的口径本身**——
>   版本号改为**里程碑驱动**（台账为机器真源 + `make release` 显式落定），
>   `make bump` 废除，同时置 `major_version_zero = true` 作安全网。
>   理由：① 只能保证"越不过 `1.0.0`"，**仍会让普通 `feat:` 自动到达 `0.1.0`**，
>   而 `0.1.0` 按所有者口径必须表示"能跑起来、能 CLI 交互"。
...
> - **A-17 进展（2026-09-20）**：该条目原有的"附带更正"（"`0.1.0` 属尚未到达的里程碑"）已过期——`0.1.0` 当日由 `make release VERSION=0.1.0` 落定（提交 `a8a30e8`，四副本一致），下一里程碑 `0.2.0` 已在 `5648bde` 补入台账。已在 A-17 行末尾追加【2026-09-20 进展】，未改写原结论（见 A 表 A-17）。
> - **新增 A-18（2026-09-20）**：`git-workflow.md` §5.2 原注声称"台账与文档表不一致会被 `tests/unit/test_release_policy.py` 拦下"——实测该声称**不成立**（该用例仅在第 1 行 docstring 引用本文件、不解析它，全仓无断言比对 §5.2 表与 `[tool.lowspec.releases]` 台账），属 S-8 失效形态（声称有机器检查、实际没有）。**已更正声明**（提交 `1e976df`）：§5.2 改为如实写"人工转写、当前无机器检查、漏改不报错、落成检查属 F 类须事先确认"。⚠️ 这是"更正了错误声明"，不是"问题已解决"——同步仍人工，落成检查待所有者裁决（见 A 表 A-18、§D 处置表）。
> - **新增 A-19（2026-09-22）**：`test-environments.md` 的 `P-1`（成果保护规则）**明文推荐**
>   `git add -A && git commit` 或 `git stash`——在**同一工作区跑多个会话**时，这两条会把他人的
>   未提交改动一并暂存 / 收走，属**反向建议**（与 A-3/A-4 同族：**按文档执行会做出错误动作**）。
>   **已修正**（源文件改为点名路径入库 + 限定路径 stash），并**补上此前完全缺失的多会话并发纪律**
>   （[`git-workflow.md`](../engineering/git-workflow.md) §6、[ADR-0022](../adr/0022-multi-session-concurrency-discipline.md)）。
>   ⚠️ 这是"**补一条从未存在的规则 + 修正一处反向建议**"，不是"修了某个实现缺陷"。
...
| **A-17** | `docs/engineering/git-workflow.md` §5「**Phase 0 ~ 首个稳定版之前**统一使用 `0.x.y`，此阶段 `MINOR` 可以包含不兼容变更」 | `pyproject.toml` 的 `[tool.commitizen]` **未设 `major_version_zero`**（默认 `False`，`commitizen/defaults.py:109`），而 `BUMP_MAP` 把 `^.+!$` / `BREAKING CHANGE` 映射为 `MAJOR`（`defaults.py:131-132`），`commands/bump.py:150-154` 按该开关选表 ⇒ 在 `0.1.0` 上出现 `feat!:` 会 bump 到 **`1.0.0`** | 与文档承诺的 Phase-0 版本语义**相反**：文档说"`0.x` 内由 `MINOR` 吸收不兼容变更"，配置会在首个破坏性提交时直接跨到 `1.0.0`（而 `1.0.0` 在本项目语义里代表"首个稳定版"） | **✅ 已处置（2026-09-19，[ADR-0019](../adr/0019-release-and-version-policy.md)）**：裁决为"**改掉产生该问题的口径本身**"——版本号改为**里程碑驱动**：台账（`pyproject.toml` 的 `[tool.lowspec.releases]`）为唯一机器真源、`make release VERSION=x.y.z` 显式落定、4 处副本一致性由 `tests/unit/test_release_policy.py` 机器检查、**`make bump` 废除并拒绝执行**；同时置 `major_version_zero = true` 作安全网（双保险）。**未采纳**：只加配置（①，仍会被普通 `feat:` 自动推到 `0.1.0`）、只改文档（②，无约束力）、`dynamic = ["version"]` 单副本化（需改动刚验过的构建链，记为后续可选）。**附带更正**：版本号由 `0.1.0` 改为 `0.0.0`（按新口径，`0.1.0` 属**尚未到达**的里程碑）。**【已验证】**：本轮实测 `git tag` 列表**为空** ⇒ "`cz bump` 从未真正跑过"这一**推断成立**。**【2026-09-20 进展】**：那句"`0.1.0` 属尚未到达的里程碑"只在 2026-09-19 当时成立，现已过期——`0.1.0` 已于当日由 `make release VERSION=0.1.0` 落定（提交 `a8a30e8 chore(release): 0.1.0`；`pyproject.toml` 的 `[project].version` / `[tool.commitizen].version`、`src/agent_sec_perf/__init__.py` 的 `__version__`、`uv.lock` 四处副本一致，由 `tests/unit/test_release_policy.py` 机器检查），且下一个里程碑 `0.2.0` 已在 `5648bde` 补入 `[tool.lowspec.releases]` 台账。本段只追加进展登记，**不改写**上方 2026-09-19 的原有结论与历史表述（报告是历史记录，改了即失真）。 |
| **A-18** | `docs/engineering/git-workflow.md` §5.2 原注「上表是**引用**（便于阅读）；两处不一致时**以台账为准**，并由 `tests/unit/test_release_policy.py` 的断言拦下」 | 实测 `tests/unit/test_release_policy.py` **只在第 1 行 docstring** 里引用本文件（"`docs/engineering/git-workflow.md` §5"），**不解析**它；该用例实际只读 `pyproject.toml` / `src/agent_sec_perf/__init__.py` / `uv.lock` / `CHANGELOG.md`（并加载 `scripts/release_edit.py`），且全仓**没有任何**断言比较 §5.2 的表格与 `[tool.lowspec.releases]` 台账（读 §4 分级一致性的是另一个用例） | 属 **S-8 失效形态**——"声称有一个机器检查，而该检查不存在"：会让人以为"台账改了、文档表漏改"会被门禁拦下，实际漏改**不会报错**，文档表与台账可悄然分叉；与 A-11/A-15/A-16 同族（文档声明 ≠ 门禁执行），只是发生点在**发布口径同步**上 | **✅ 已更正声明（2026-09-20，提交 `1e976df`）**：§5.2 改为如实表述——上表是**人工转写**的引用、**当前无机器检查**比较它与台账、改台账时必须人工同步本表；并写明若日后把"同步"落成机器检查，属改门禁强度（`F` 类）须事先确认。**注意：这是"更正了错误声明"，不是"问题已解决"**——台账↔文档表的一致性目前仍靠**人工同步**，落成机器检查待所有者裁决（见 §D 处置表）。残留风险：人工同步可能漏改而不报错 |
| **A-19** | `docs/engineering/test-environments.md` §4 规则 `P-1`「进入 T2/T3 之前，工作树必须干净 \| `git status --porcelain` 为空；否则先 **`git add -A && git commit`** 或 `git stash`」 | 全仓**没有任何**"多会话"（同一工作区同时跑多个会话）的提交 / 推送纪律：`git-workflow.md` 与 `.codebuddy/rules/git-workflow/RULE.mdc` 只写"本地 `add`/`commit` 可在当前任务范围内进行"，**未禁止** `git add -A` / `git add .` / `git commit -a` / 不带 `-o` 的 `git commit` / 不带路径的 `git stash` | 同一仓库的**索引与工作树是全仓共享的单例资源** ⇒ `git add -A` 会把**其他会话**未提交的改动一并暂存（`git stash` 一并收走），此后任何一次不带 `-o` 的提交即把它们推上远端 ⇒ **commit 内容失真，且事后无法从仓库判断是否发生过**。与 A-3/A-4/A-11/A-15 **同族**（文档声明的做法与实际该做的相反），但形态更隐蔽：它不是"声明了一个不存在的检查"，而是**直接教了一个有害命令** | **✅ 已修正（2026-09-22）**：① `P-1` 改为**点名路径**入库（`git add <路径>` + `git commit -o -- <路径>`）/ **限定路径**暂存（`git stash push -- <路径>`），并写明禁令与理由；**同一次反向搜索另发现 `testing-strategy.md` §6 的"对照组…可用 `git stash`"属同类反向建议**，一并改为"优先在上一提交上测 / 独立快照；确需暂存时限定路径"；② 补上此前**完全缺失**的多会话并发纪律——[`git-workflow.md`](../engineering/git-workflow.md) 新增 §6（规则 M-1~M-9，含**推送范围隔离**与**推送被拒时的唯一合法路径**）、常驻摘要 `RULE.mdc` 同步、`CODEBUDDY.md`/`AGENTS.md` §2 补常驻结论、`agent-teams.md` §6 划清"会话内成员 vs 会话之间"的靶子边界、决策落 [ADR-0022](../adr/0022-multi-session-concurrency-discipline.md)。**边界**：该纪律**不可由 CI 强制**（CI 只看到一个 runner、看不到"是否存在多个会话"）⇒ 记为**部分缓解**，不得表述为机制。**未做**（属 `F` 类，待所有者裁决）：把"四处口径一致 + 禁令命令集一致"落成机器检查（ADR-0022 §8） |
| **A-20** | [`product-walkthrough.md`](product-walkthrough.md) §A 第 3 条与 §B 第 22 行：「手册 §1.4 把"无沙箱"的后果写成"（`T-04` 仍未缓解）"，与 `architecture.md` §7.2 的落位（`T-01`/`T-13`）**不一致**……该差异**已在本次回报中提请团队领导核对**」 | 该不一致**早在 2026-09-21 就已修正**：手册 §1.4 已按"更正而非抹掉"拆成两条（沙箱 ⇒ `T-01`/`T-13`；`T-04` 单列并写明**与沙箱无关**），经过见 [`docs/devlog/0020`](../devlog/0020-2026-09-21-M1出口推进与手册独立复核.md) §3.8 | 这两处**读起来像"问题仍悬置"**，会让人**重核一遍已经核过的事**（而那次核对的结论就是要以 §7.2 为准）⇒ 与 A-12/A-18/A-19 **同族**（陈述落后于仓库），形态是"**已闭合的事项仍以未闭合语气呈现**"；对"跑测导引"这类**被当作脚本执行**的文档尤其有害（§C.3 的硬边界正要求助手据它判读） | **✅ 已更正（2026-09-23，提交见 `docs/devlog/0021`）**：两处均就地追加 📌 更正块（写明**已闭合**、指向 0020 §3.8 与手册 §1.4 现写法、并注明"保留原文仅为留痕，不要据它去核对手册"）。**顺带的核对**：全仓 `grep` 确认**无**其它文档引用这两处 ⇒ **无悬空引用** |
...
| A-18（流程文档的虚假机器检查声明） | 首席架构师（源文件 `git-workflow.md` §5.2 属其域；记录员登记本核查） | 声明已由 `1e976df` 更正；补机器检查属 `F` 类、待所有者裁决 |
| A-19（流程文档里的**反向建议** + 一处规则空白） | **已直接修正** `test-environments.md` `P-1`；`git-workflow.md` §6 与 `ADR-0022` 属**首席架构师**域、`CODEBUDDY.md`/`AGENTS.md` §2 属**常驻结论** | `test-environments.md` §4；`git-workflow.md` §6；`docs/adr/0022-multi-session-concurrency-discipline.md`。补机器检查（`F` 类）待所有者裁决 |
| A-20（**已闭合事项仍以未闭合语气呈现**） | 记录员登记；源文件 `product-walkthrough.md` 已由**团队领导直接更正**（属**事实更正**：不动任何判据、不改威胁状态与计数） | [`docs/engineering/product-walkthrough.md`](product-walkthrough.md) §A 第 3 条 / §B 第 22 行 |
