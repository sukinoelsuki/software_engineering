...
- **对照组**：必须包含改动前的基线（**优先**用"在上一提交上测得"，或在**独立快照**里测得；
  确需暂存时用 `git stash push -- <自己的路径>`——⚠️ **不要**用不带路径的 `git stash`：
  工作树是全仓共享的，它会一并收走**其他会话**未提交的改动，见
  [`git-workflow.md`](git-workflow.md) §6「多会话并发纪律」）。
