...
>
> **`C7` 实测（第二轮 · 2026-09-25，`sn=cnb-hlo-1k3c9o8gu`）**：设置源**拆分后**的首个环境，
> `Loaded 38 custom blacklist commands`（与源文件实测 38 条一致）/ 12 类禁用（不含 `custom`）/
> `isAutoExecuteTerminal: true` 三项齐备；同批 stage `agent-permissions` = **success / 0.3 s**，
> 两目标各 `[ok] 已合并 75 项设置（68 + 7）`，两个 User 目标键集合完整（WebIDE 86 / Remote-SSH 75）
> ⇒ **"33 → 38"与设置源拆分均已落地**（判据见 §3.1）。
> ⚠️ 同批实测见 §6 边界 9：客户端连接后存在**第二个改写者**（合并语义，只规范化 `extensions.autoUpdate`）。
...
| 平台覆盖**晚于** stage | 环境刚启动时读 `User/settings.json`，再跑 `C4`，对比 `codingcopilot.autoRun` 是否存在 | 2026-09-25 实测：**确有晚于全部 stage 的改写**（客户端连接后 ≈7 s），但**语义是合并** ⇒ 我方 `codingcopilot.*` 键**未被覆盖**（`C7` 仍读到 38 条）；改写主体未确认，且经所有者决定**不再专程验证** ⇒ 见 §6 边界 9 与 ADR-0032 §11（`U4`） |
...
| 9 | **运行期可能存在第二个改写者（语义 = 合并，非整体覆盖）** | 2026-09-25 实测（`sn=cnb-hlo-1k3c9o8gu`）：客户端连接后约 7 秒（`21:02:49` → `21:02:56/57`，**晚于全部 stage**），code-server 的 `Machine` 与 `User` 两份设置被**再次改写**。改写**保留**我方 75 键，只把 `extensions.autoUpdate` 规范化为该客户端合法的 `"off"`（源里写 `false`）⇒ ① **闸门键不受影响**（`C7` 仍读到 38 条）；② **凡"我方取值与客户端枚举不一致"的键，运行值可能与仓库源不同**——判此类问题必须读**运行中的文件/会话**，不能只看源（本节第 5 条家族）。改写主体**未确认**，见 devlog 0025 §5 第 8 条；⚠️ **所有者 2026-09-25 决定不再专程验证该主体**（对照配方归档于 [devlog 0026](../devlog/0026-2026-09-25-收尾承接与运行期改写观察项.md) §2.2） |
...
- **2026-09-25（第二批 · 环境重建后实测）**：更正**命令链切分口径**——单 `|` **不是**切分符。
  证据：扩展产物 `out/extension/index.js` 的 `splitCommandChain` 为 `split(/(?:\|\||&&|&|;|\n)/)`；
  实机探针 `echo aGVsbG8= | base64 -d | cat` 命中 `\bbase64\b…\|` 并弹出确认（`sn=cnb-174-1k3c5dnng`）。
...
- **2026-09-25（第四批 · 设置源拆分后首个环境的落地核对）**：环境 `sn=cnb-hlo-1k3c9o8gu`
  （`develop@18d9ea5`）内核对通过——stage `agent-permissions` = success（0.3 s，两目标各
  `[ok] 已合并 75 项设置（68 + 7）`）；`C7` 读到 `Loaded 38` / 12 类（不含 `custom`）/
  `isAutoExecuteTerminal: true`（扩展版本仍为 `4.12.38765564`）；两个 User 目标键集合完整
  （WebIDE **86** = 我方 75 + 平台自有 11；Remote-SSH **75** = 与源逐键一致）。
  新增 §6 边界 9：客户端连接后存在**第二个改写者**（合并语义；只把 `extensions.autoUpdate`
  规范化为该客户端合法的 `"off"`）。
- **2026-09-25（第五批 · 任务结束的收尾）**：所有者决定**不做**"有 stage / 删 stage"的对照 ⇒
  §6 边界 9 标注"**不再专程验证**改写主体"（对照配方归档于
  [devlog 0026](../devlog/0026-2026-09-25-收尾承接与运行期改写观察项.md) §2.2）；
  §5 `D1` 的"平台覆盖晚于 stage"一行更新为**本轮实测口径**——确有晚于全部 stage 的改写，
  但**语义是合并** ⇒ 我方 `codingcopilot.*` 键**未被覆盖**。
  ⇒ 边界 9 的"运行值可能被规范化"**不变**，而"平台是否会覆盖我方闸门键"这一疑虑已有实测答案（**否**）。
