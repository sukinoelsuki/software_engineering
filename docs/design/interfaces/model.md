...
#### 2.3.1 自查结论（2026-09-18，逐处核对 `ADR-0011` / `SRS` 后得出）

> 本节回应"能否直接填充成员（例如用已定义的 `S/M/L`）"这一问题。
> 结论：**不能填充 `S/M/L`**【待定】维持。依据如下（均为原文，可逐条复核）。

**（1）`S/M/L` 确已定义，但它属于"硬件档位"这条轴。**

- `ADR-0011 §5.1`（标题即「**修订后的**档位矩阵（取代 ADR-0010 §5.4）」）给出 `S/M/L` 三档，
  其**决定量是内存预算**：`S ≤ 4 GiB` / `M 8 GiB` / `L 16 GiB`，并各配一个预置模型
  （`MiniCPM5-2B` / `Qwen3-4B` / `Qwen3-8B`）。
- `SRS §6.3` 把同一张表放在标题「**硬件适配的目标范围**」之下。

  > 原文（`SRS §6.3`）：「**本期适配目标** | **笔记本电脑与个人 PC**（异构但可枚举：内存 8~64 GiB、
  > 核数 4~24）」……「**档位构成**（三档全部可在当前云开发环境验证）」

- `SRS §14` 术语表**逐字**区分两条轴：

  > 原文（`SRS §14`）：「能力档位 | 对**模型能力**的粗略分级，用于选择工具集与提示结构。
  > **与硬件档位是两个独立的轴**」；「**硬件档位（S/M/L）** | 对**可用硬件预算**（内存、核数）的分级，
  > 用于选择模型规模、量化档、上下文长度与并行度。**不可由模型能力推导，反之亦然**」

- `ADR-0010 §5.2/§5.3` 同义：`工具集大小与描述详略（REQ-HARNESS-03）` 由**模型能力档位**决定，
  且「**同一模型在不同硬件上能力档位不变**」。

⇒ `S/M/L` 是**硬件**档位的成员。把它填进 `CapabilityTier` 等于**合并两条轴**，
正是上述两处原文明确禁止的操作。

**（2）"模型能力档位"的成员在**任何**文档中都没有定义。**

- `SRS Q-3` 是**开放问题**：

  > 原文（`SRS Q-3`）：「能力档位如何划分？自动探测还是手动配置？」，"建议"列仅写
  > 「建议先**手动 + 探针辅助**，档位暂定 **3 档**」——是**建议**，不是决策。

- `REQ-MODEL-06` 只要求「通过探针任务**估计**当前模型能力档位」（验收：与人工评估一致率 ≥ 80%），
  **不给档位集合**；`REQ-MODEL-03`（统一抽象）与 `REQ-MODEL-05`（路由降级）与档位划分无关。

⇒ **无定义可填** ⇒ 保留【待定】。卡点：① 档数（3 还是 4）；② 判定口径（探针任务集 + 阈值）。

**（3）误填诱因（本文件仅登记，不改 ADR 原文）。**

`ADR-0010 §5.4` 的表**标题**写作「**模型档位**矩阵（含新增的大一档模型）」，但其内容是
`S/M/L` 的**硬件**档位表（内存预算 + 预置模型）——与同一 ADR §5.2 称该轴为「**硬件**档位」
