...
| 错误语义 | `emit` / `flush` 失败**必须冒泡**；**禁止**吞异常或降级为告警。<br>**【2026-09-22 措辞写死，仅增不改】"冒泡"的具体形态** = 由**实现层**抛出其自有契约级异常 `AuditWriteError`（`foundation/errors.py`，底层 `OSError` 保留在 `__cause__`）；**禁止**在捕获点改写异常类型（即不得把 `AuditWriteError` 再包装成别的类型）。<br>⚠️ **为什么必须写死**：只写"必须冒泡"而不写"由谁抛什么类型"，下游只能用 `except Exception` 一把抓 ⇒ 会把"证据面坏了"收敛成"任务失败"（威胁模型 §8.2 的 `P-3`）。写死后可**按类型**判定，不经 CLI 的消费者同样可判定 |
