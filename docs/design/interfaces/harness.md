...
| R3 | gate **抛异常** | 该调用**不执行** + 记 `ERROR(INTERNAL, text="人工确认通路故障")` + **终止任务**（`TASK_FINISHED(FAILED)`）。理由：审批通路坏掉 = 权限判定的**输入面**不可用；继续跑等于在未知权限语义下继续。**不得**吞掉异常后当成一次普通拒绝（那会把基础设施故障伪装成默认拒绝，`policy.md` §2.5 的同一取舍）<br>⚠️ **【2026-09-22 追加，仅增不改】唯一例外 —— `AuditWriteError` 不适用 R3**：若异常是 `AuditWriteError`（审计写入失败），它**不是**"审批通路故障"而是"**证据面损坏**" ⇒ 按 [`audit.md`](audit.md) §2.4「必须冒泡」，**原样抛出**、逃逸出 `run()`，**不得**收敛成 `ERROR(INTERNAL)` + `TASK_FINISHED(FAILED)`。理由与完整代价见威胁模型 §8.2 的 `P-3`：<br>① 两者**性质不同**（证据面坏了 ≠ 任务没走通）；<br>② 同形会让消费者去查任务逻辑而不是磁盘，**且落盘的事件流会把这次事故永久记成"任务失败"**——证据文件自己不知道自己是坏的；<br>③ 失败方向**不变**：仍是"不执行 + 终止"，只是**不伪装成任务失败**。<br>**实现**：`harness/loop.py::_request_approval` 在 `except Exception` **之前** `except AuditWriteError: raise`（顺序反了则永远不生效） |
...
| 2026-09-19 | **第十三版（复核领导代改的第八/十/十一版 + `T6` 落地一致性：逐项读源码与用例，**四项均确认、无需更正**）**：① **第八版**（`7721658`，`I8`/`V4` 的键名判据"由谁产生"）**与实现一致**——`harness/arguments.py` 的 `_invalid()` 是唯一消息来源，已声明键名经 `sanitize_for_display(limit=_KEY_LIMIT=32)` 回显（`_require_present_keys` / `_validate_value`），**未声明键名**（`_reject_unknown_keys` 走 `key=None`）与**参数值**一律不回显；`loop.py` 的 `_VALIDATOR_DETAIL_LIMIT = 200` 与该版记录相符。⚠️ 该版末句"未声明键名这一面**尚无用例**"**已解除**：`tests/security/test_harness_s1c_argument_leak.py::test_s1c_undeclared_key_name_with_control_chars_not_leaked` 已入库（另有已声明键的正例与"取值仍不泄漏"面）。② **第十版**（`ADR-0020` 批准后的表述翻转 + 编号更正）**已兑现**——`ADR-0020` 状态行「已接受」、`docs/adr/README.md` 索引「已接受（2026-09-19）」、本文件「提议中 / 未批准前不得开工」表述已全部翻转；编号更正落实（第八版 = `I8`/`V4`、第九版 = 复核行，本文件内引用处已作「第九版」）。③ **第十一版**（`I8` 与字段表的矛盾 ⇒ **按位置分判据**）**与实现一致**——§2.2 的 `I8` 澄清表（独立字段 ❌ / `text`·审计·日志 ❌ / `response` 内部 ✅ / 渲染 ❌）与 §2.6 第 3 条一致；`harness/loop.py` 把完整 `ModelResponse` 放进 `MODEL_RESPONSE` 事件，`tests/unit/test_harness_loop.py` 的 `I8` 判据是**结构性**的（sentinel 不进 `text`/`tool_name`/审计 `detail`；`arguments_summary` 内无 `arguments_json`）。④ **`T6` 三处一致**——`contracts/tools.py`（`specs()` docstring =「全量注册集（未裁剪）」）、`tools/registry.py::specs`（同）、`harness/loop.py`（"已裁决（全量）"）、`cli/app.py`（按全量使用）四处口径一致；`tests/unit/test_harness_internals.py` 的 `LEAF_UNITS` **已含 `arguments`**（`H2` 对新模块生效）；已落地的 `harness/arguments.py` 按 `spec.parameters_schema` 校验、不经 `specs()`，**无冲突**。**§2 的类型 / 成员 / 不变式 `I1`~`I10` 一律不变**；`contracts/harness.py` **无需改动**。 | 逐项读源码与用例核实：`harness/arguments.py`、`harness/loop.py`（L40/L120-121/L296）、`contracts/tools.py`（L96）、`tools/registry.py`（L118）、`cli/app.py`（L368）、`tests/unit/test_harness_internals.py`（`LEAF_UNITS`）、`tests/unit/test_harness_loop.py`、`tests/security/test_harness_s1c_argument_leak.py`、`docs/adr/README.md`；`git log`（`7721658` / `43a177c` / `df55cdb`） |
