...
| U9 | [`harness.md`](harness.md) 引入的未决项（**不阻塞开工**，但**不得默认处置**/**不得按已实现表述**） | ① **参数校验器选型** ⇒ **已提出 [`ADR-0020`](../adr/0020-argument-validator-implementation.md)（提议中，待领导批准）**：4 个候选 + 加权对比 + 推荐**手写 JSON-Schema 子集校验器**，并登记 `ADR-0015` 的 `D2` **两处用途均不落地**（**新增 ADR**；`ADR-0015` 正文不改、只追加修订记录指针）。⚠️ **批准前不得开工**（`harness.md` §3.4 / §5.1 第 8 行 / §7.2）；② **"总是允许"的持久授权**（= `U3`）：本轮 `ALLOW_ALWAYS` **等价于** `ALLOW_ONCE`，但必须在事件与审计里**如实记录** `allow_always`；③ **检查点持久化落点**（`REQ-HARNESS-05`）：`checkpoint.py` 本轮只做纯函数快照/还原，落盘路径与权限未决——任何落盘**必须**沿用 [`audit.md`](audit.md) §2.5 的口径（根不得来自不可信来源、先校验后创建）；④ **领域包根**：`load_pack(roots=...)` **必填无默认**，装配点给哪些根未决；⑤ ~~实现侧差异 `R-1`~`R-4` 待裁决~~ ⇒ **已裁决（2026-09-19，`c3d5610`）**：`R-1` 以契约为准、**改实现**（轴用 `CapabilityTier`，**不新增 ADR**）；`R-2`/`R-3` **采纳实现**、契约已收紧（`prompts` 四 builder + `data_context` 角色守卫；`ErrorDisposition{RETRY, FEEDBACK, ABORT}` 留 `harness/errors.py`，新增 `error_kind`）；`R-4` **保留契约**（`not_exposed` / `unknown_tool` 必须可分） | 见 [`harness.md`](harness.md) §3.4 / §2.5.5 `R6` / §3.1（`checkpoint.py`）/ §4.1 `P4` / §7.1（裁决落地清单）/ §8 的 `R-1`~`R-4`；逐项解除时走本目录 §7 的变更流程（触及决策则另开 ADR） |
