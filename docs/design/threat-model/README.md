...
> **缺行为层验证 12 条**（唯一例外是 `T-02` 的"拒绝"半，口径与清单见 §5）；
...
**13 条中 12 条缺行为层验证**（2026-09-18 前为 13 条；`T-02` 的"拒绝"半是本次唯一的例外）。
**口径**（本文统一，避免数字漂移）："缺行为层验证" = 该条目**没有任何行为层（对抗性）用例**；
`T-01`/`T-02`/`T-06` 另有**结构性**机器检查，但**结构性检查不构成行为断言**——
它证明"代码长成约定要求的样子"，**不证明"攻击被挡住"**。清单如下：
