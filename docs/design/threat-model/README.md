...
> **2026-09-25 新增 `T-15`（Agent 命令闸门放宽为提醒层，依据 `ADR-0032`）**。
> **15 条威胁条目**——**已缓解并验证 1 条 / 部分缓解 12 条 / 未缓解 2 条**
> （`T-14` 于 2026-09-25 新增：数据与读数的静默损坏，见 §4 与 §7）
> （`T-15` 于 2026-09-25 新增：开发期命令闸门放宽，见 §4 与 §7）
...
1. **安全设计是齐的；实现载体已部分到位，但多数缓解仍无可执行的行为级验证。** 需求
   ...
   ⇒ 14 条里 **2 条未缓解**（`T-04`/`T-10`；**计数与 §4.1 一致**，
...
**覆盖范围**（本轮）：可执行路径 + 工程流程 + 供应链 + **开发期工具链**，共 **15** 条
（`T-15` 于 2026-09-25 新增：**开发期**的命令闸门，与 `T-11` 的**会话内工具授权**是两个落点，见该条目顶注）。
...
协作流程（共享 git 索引、子代理输出）、**开发环境（IDE 内主 Agent 的命令执行闸门，`T-15`）**。
...
flowchart LR
    subgraph UNTRUSTED["不可信侧（一律视为数据，不得作为指令）"]
        ...
        U6["外部依赖 / 模型权重 / 二进制"]
        U7["开发期：任何会被读进 IDE 助手上下文的文本<br/>（含仓库文件 / issue / 网页）"]
    end
...
| [`T-14`](supply-chain-and-process.md#t-14-基准数据与读数的静默损坏覆盖--错链--工具链缺失) | 基准数据与读数的静默损坏（覆盖 / 错链 / 工具链缺失） | 数据完整性（**本项目自有的类别**） | 部分缓解（2026-09-25 新增） | **有**：五条机器检查 + 两轮变异探针（1:1 失败）+ **一次真实发布端到端核对**（`sn=cnb-8g5-1k3avbsr4`：diff 只有新增、当天另一轮 124 个文件不变、能力读数 `0.0 → 0.6667`）；⚠️ 但按 §5 口径仍**缺行为层用例**（`publish.sh` 的对抗性集成用例未做） |
| [`T-15`](untrusted-input-and-agentic.md#t-15-agent-命令闸门放宽为提醒层默认全自动) | Agent 命令闸门放宽为「提醒层」（默认全自动） | STRIDE-E / 智能体特有（**开发期**命令执行面） | 部分缓解（**2026-09-25 新增**） | **部分**：17 条机器检查（`tests/unit/test_apply_ide_settings.py`：成对存在 / 33 条正则为合法 / 5 条不可逆基线必被覆盖 / 4 条"改坏必红"）；**⚠️ 全是配置层，非行为层**；**缺**：绕过面探针（`U3`）、误报基线、黑名单命中的**人工**行为核对（`C5`）；**含一处刻意取舍**：`injection` 类别已关 ⇒ `curl\|sh` **不再被拦**（不是遗漏） |
...
| 部分缓解 | **12** | T-01、**T-03**、T-05、T-06、T-07、T-08、T-09、**T-11**、**T-12**、T-13、**T-14**（2026-09-25 新增）、**T-15**（**2026-09-25 新增**） |
| 未缓解 | **2** | T-04、T-10 |
| **合计** | **15** | |

> **2026-09-25 复核（`T-15` 新增 ⇒ 分布 `1 / 11 / 2` → `1 / 12 / 2`、条目数 `14 → 15`）**：
> 依据 `ADR-0032`（Agent 命令闸门放宽，F 类授权）与其落地提交 `ec8791d`。
> **`T-15` 落「部分缓解」的理由**：其缓解（33 条黑名单 + `safeDelete` + 启动期断言 + 17 条机器检查）
> **已实现并在跑**，但**覆盖面不全**——黑名单是**文本匹配**、**可被改写绕过**，
> 且其**行为验证只能人工做**（`C5` 在功能面板里，`tests/security/` 无法承载）
> ⇒ 正落在「部分缓解」的**既定定义**（"已实现或在跑，但覆盖面不全 / 或本身是流程纪律而非机制"）。
> ⚠️ **不得**据此把「已缓解并验证」从 1 改动（`T-15` **缺口未闭合**）；
> ⚠️ **不得**把 `T-15` 读成"不可逆操作已被阻断"——它是**提醒层**（见其条目「现有缓解」的三条连读边界）。
> **`T-05`** 另加一句**联动注**（与 `T-15` 同机制、不同面：协作面对开发期命令面），
> **其状态与计数不变**（仍「部分缓解」、仍列 §5）。
...
**按此定义计数：15 条中 8 条缺行为层验证。**（**2026-09-20 重算**：由 8 降为 6——
`T-03`/`T-09` 移出，依据见本节的 2026-09-20 复核段；**旧值 8 保留**于下方计数沿革；
**2026-09-25 新增 `T-15` ⇒ 6 → 7**，见下方同批段）
...
**2026-09-25 新增 `T-14` ⇒ 本项计数 6 → 7。**新条目**不在**"已有行为级用例"之列：
它的缓解是**结构性机器检查**（五条，`make check` 内自动跑）+ **一次真实发布端到端观测**
（`sn=cnb-8g5-1k3avbsr4`：diff 只有新增、当天另一轮 124 个文件不变）。
按本节口径（**结构性检查不计入、人工观测不计入**）两者都不算行为级用例
⇒ 缺的正是 `publish.sh` 的**对抗性集成用例**（本地假远端连跑两轮同日不同标签），见 `T-14` 「应有」b。
**2026-09-25 新增 `T-15` ⇒ 本项计数 7 → 8。** 新条目**不在**"已有行为级用例"之列：
它的缓解是**配置层 / 结构性机器检查**（17 条单测）+ **人工在会话内的行为核对**（`C5`）。
按本节口径（**结构性检查不计入、人工观测不计入**）两者都不算行为级用例
⇒ 缺的正是其条目的「缺（行为层）」a/b（**绕过面探针 + 误报基线**），见 `T-15` 「验证方式」。
...
| T-15 | ① **绕过面**：同一破坏性意图的 N 种改写变体**逐条**记录"是否被黑名单命中"；② **误报基线**：日常命令集**零触发** | `tests/security/`（对抗性输入集）+ 会话内人工核对 | **缺**（新增 `T-15` 时一并登记）：现有 17 条是**配置层**检查（`tests/unit/test_apply_ide_settings.py`），按本节口径**不计入**行为级用例；行为侧**原则上不可由 CI 断言**（发生在功能面板，见其条目「验证方式」的 `C5`） |
...
另有 **4 项属"已缓解但覆盖不全/靠人执行"**，不建议当作机制依赖：
...
| 9 | 基准数据的落盘与读数（`T-14`） | 两条已知路径已修 + 五条机器检查；**"检出坏数据"仍是人工的**（无"读数合理性"判据），且四道闸门**靶不同**、不覆盖本条 | 见 `T-14`「应有」：① 发布前读数合理性闸门（能力全 0 且 `mypy_rc` 全 1 ⇒ 拒绝入库）；② `publish.sh` 对抗性集成用例（本地假远端同日两轮） |
...
- **2026-09-25（新增 `T-14`；依据 `ADR-0026`、`devlog 0024`）**：本条由**一次真实发布前的
  前置核对**发现——`bench/data` 上当天已有一轮（`daily/2026-09-25/`，124 个文件、其中 120 份原件），
  而发布脚本对目标目录是"先删后拷"、轮次目录又按**日期**命名 ⇒ 同日第二轮会**删掉第一轮的原件**，
  且索引里第一轮的 `report` 仍指向该目录（见 `ADR-0026` §1）。
  发布验收跑通后**又发现第二条同族路径**：评测要用的 `mypy` 未安装（`[project.optional-dependencies].dev`
  只在 `make setup` 时装）⇒ **能力通过率恒为 0%**，而四道闸门全过、构建全绿、报告写着"无告警"。
  ⇒ 两条路径的共同点是"**不触发任何红灯**"，故入账为一条威胁（**数据与读数的静默损坏**），
  而不是两次孤立的 bug。
  **数字变化（唯一一次，逐一列明）**：条目数 **13 → 14**；§4.1 部分缓解 **10 → 11**；
  §5 缺行为层验证 **6 → 7**；**"已缓解并验证"仍为 `1`**——
  ⚠️ **不得**因"本次有机器检查与变异探针"升级任何条目（本条的缓解是**结构性**检查，
  按 §5 口径不计入行为级证据；缺的那条见 `T-14`「应有」b）。
  **本轮做过的事**：修复两条路径（`ADR-0026` + `.cnb.yml` 的 `prepare` 阶段）、
  新增五条机器检查（含两轮变异探针，其中一条**第一版恒过**并被当场抓出）、
  一次真实发布的端到端核对（`sn=cnb-8g5-1k3avbsr4` / `sn=cnb-til-1k3b07aqj`）。
  **本轮不做的事**：不改任何既有 `T-XX` 状态、不动 `§8` 的裁决登记、不为本条补行为级用例
  （需假远端夹具，登记为缺口）。

- **2026-09-25（新增 `T-15`；依据 `ADR-0032`、落地提交 `ec8791d`）**：新增一条威胁条目
  **「Agent 命令闸门放宽为『提醒层』（默认全自动）」**——**开发期**主 Agent 执行的命令面。
  ① **为什么单列**：`T-11` 的落点是**会话内 `Tool.invoke` 的授权链**（`PolicyEngine` / `Capability` /
  `AuditSink`，**产品运行时**）；`T-15` 的落点是**开发期**主 Agent 在开发环境里执行 shell
  （**第三方扩展的配置**，其缓解**不在 `src/` 里**）⇒ 两者**承载机制完全不同**，
  并入会让 `T-11` 的"工具滥用"半更含糊。**本条目只登记本面，不重复 `T-11` 的论证。**
  ② **状态 = 「部分缓解」**：缓解（33 条黑名单 + `safeDelete` + 启动期断言 + 17 条机器检查）
  **已实现并在跑**，但**覆盖面不全**——黑名单是**文本匹配、可被改写绕过**，
  且其**行为验证只能人工做**（发生在功能面板，`tests/security/` 无法承载）。
  按 §4.1 定义正落在该档；⚠️ **不得**据此改动「已缓解并验证」（保持 `1`）——
  其缺口（**绕过面探针 `U3`** 与**误报基线**）未闭合。
  ③ **一处刻意取舍（必须连读，防误读为"已缓解"）**：`injection` 类别已关 ⇒ **`curl | sh` 不再被拦**
  （它是最主要的误报源，**故意不拦**，恢复只需删一行）；黑名单命中 = **弹确认**（`ask`），
  **不是**直接拒绝、**也**不是安全边界。
  ④ **它同时引入两个新的"配置面"失败模式**（已由机器检查堵住，但检查本身需维护）：
  设置文件 ↔ 断言列表、设置文件 ↔ 启动期 stage（删 stage 即**静默失效**）。
  **数字变化（逐一列明）**：条目数 **14 → 15**；§4.1 部分缓解 **11 → 12**；
  §5 缺行为层验证 **7 → 8**；**"已缓解并验证"仍为 `1`**。
  ⑤ **`T-05` 加一句联动注**（与 `T-15` 同机制、不同面：**协作面** vs **开发期命令面**），
  **其状态与计数不变**。**本轮未动**：其它 `T-XX` 状态、§4.1 的三档定义原文、§8 的裁决登记、
  任何 ADR 的既有正文。
  **文档落点**：`ADR-0032`（决策与根因）、`docs/engineering/agent-command-gate.md`（生效路径、复跑、故障判据）。
