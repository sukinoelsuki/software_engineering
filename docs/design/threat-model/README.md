...
> **2026-09-20 状态更新（所有者裁决 `P-2`，见 §8.2）**。
> **13 条威胁条目**——**已缓解并验证 1 条 / 部分缓解 9 条 / 未缓解 3 条**；
> **首条「已缓解并验证」= `T-02`（2026-09-20 所有者裁决）**，⚠️ 它是**单条目、单调用面**的结论
> （只覆盖**会话内工具层**的"拒绝 + 且留审计"；配置期 / 装配期调用点按 `Q-1` 属**范围外**）
> ⇒ **不得**读成"安全已到位"：**未缓解仍是 `T-03`/`T-04`/`T-10`**；
...
| 部分缓解 | **9** | T-01、T-05、T-06、T-07、T-08、T-09、**T-11**、**T-12**、T-13 |
...
> **不证明**整体安全——**未缓解仍有 3 条**（`T-03`/`T-04`/`T-10`）、**9 条部分缓解**的缺口各照旧，
...
- **2026-09-20（所有者裁决落地 · `P-2` 升级 + `Q-1` 不纳入 · **本项目首次出现「已缓解并验证」**）**：
  ...
  **后果**：**零 ADR、零新模块**（原候选 (i)/(ii)/(iii) 均不再需要）、**不改 `R1`**（分层不动）；
  ...
  ⑥ **一处未同步（如实登记，待领导裁决）**：§8.2 的 `Q-2` 正文仍写「现状：未决」，
...
> `detail.reason="path_not_allowed"`**；三条理由见 `docs/devlog/0019` §3.10）。
> ⚠️ **`Q-2` 正文末尾的「现状：未决」是 2026-09-19 当时的记载，按"只增不改"保留**，
> **以本注为准**；`P-2` 的缺口 (c) 即随该裁决**不再构成阻塞**。
>
