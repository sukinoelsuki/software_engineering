...
> **缺行为层验证 12 条**（**口径已写死于 §5**，并说明与初稿数字的差异来源；唯一例外是 `T-02` 的"拒绝"半）；
...
2. **真正生效的安全机制有两类：结构性机器检查 + 第一组行为层用例。**
   ...
   - **行为层（2026-09-18 新增，`f436b0b`）**：`tests/security/` 的 **13 个 `@pytest.mark.security` 用例**
     ...
     （`make test-security` → **13 passed / 85 deselected**）。
...
| [`T-02`](execution-and-isolation.md#t-02-路径穿越) | 路径穿越（`..` / 符号链接 / 白名单逃逸） | STRIDE-E | 部分缓解 | **部分**：`S2` **拒绝**半（`f436b0b`，11 个行为用例 + 2 个接缝用例）；**审计**半无载体 |
...
**按此定义计数：13 条中 12 条缺行为层验证**（唯一例外是 `T-02` 的"拒绝"半）。
**与初稿数字的差异来源**（避免"数字看似没变"被误读）：初稿写"12 条"时把 `T-06` 当作
"已有机器检查"而**未计入**；但 `T-01` 同样已有结构性机器检查，且两者的检查都**不是行为断言**
⇒ 按本定义 `T-01`/`T-06` **都应计入**；同时 `T-02` 因 `S2` 拒绝半落地而**移出**。
两个方向的移动相抵，故本次仍为 **12 条**（若把同一新定义回溯到 `f436b0b` 之前，则为 **13 条**）。
清单如下：
