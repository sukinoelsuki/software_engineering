...
**状态**：`已缓解并验证`（**2026-09-20 由「部分缓解」升；所有者裁决 `P-2`——本项目首次**，
见 `README.md` §8.2 的【裁决后状态】与下方「2026-09-20 升级」；
2026-09-18 重评：`S2` 的"**拒绝**"半已有可执行行为用例；
**2026-09-19 更正**：原写"**且留审计**"半**无载体**——**该表述有误**，承载点与**两处行为层证据**
均已存在，见下方「2026-09-19 更正（审计半）」）｜ **主导类别**：STRIDE-E（权限提升）
...
> **【2026-09-20 追加，仅增不改】**上一句是 2026-09-19 当时的记载；`P-2` **已裁决为「升级」**
> ⇒ `T-02` = 「已缓解并验证」（见下「2026-09-20 升级」）。
>
...
> **【2026-09-20 追加，仅增不改】**`Q-1` **已裁决为「不纳入」**（这一调用面属**范围外**：
> fail-closed、无审计）⇒ 上句的"待裁决"**已结**；规范见
> [`../interfaces/audit.md`](../interfaces/audit.md) §2.6。
>
...
>
> **2026-09-20 升级（所有者裁决 `P-2`，裁决登记见 `README.md` §8.2）——本项目首条「已缓解并验证」**：
> **依据** = 上述两半的**已入库、可回归、带变异探针**的行为层证据（证据映射原文见 `P-2`）：
> - 「**拒绝**」半：`test_path_traversal_rejected.py`（**18 例**）+ `test_path_validation_seam.py`
>   （2 接缝例），含**变异验证**（判据换成字符串前缀式 ⇒ 4 个前缀欺骗用例**全部翻红**）；
> - 「**且留审计**」半：`test_s1_replayable_audit_link.py::test_denied_tool_call_is_audited_and_replayable`
>   与 `test_t02_path_traversal_audit.py`（**两处真实 `JsonlAuditSink` 端到端 + 变异探针**）。
>   剩余缺口 (a)~(c)（原 `P-2` 所列）经所有者判定**均不阻塞**：(a) 攻击面 6 的 `W1`~`W8` 全部落地、
>   UNC 已判**不构成独立绕过面**；(b) 配置期 / 装配期按 **`Q-1` 为「范围外」**；(c) 审计 `outcome`
>   已由 **`Q-2`** 裁决**维持现口径**。
>
> **三条残余风险（升级后仍然存在，必须与本条状态连读）**：
> ① **`UNC` / 盘符语义跨平台未验**——Linux 下前导 `//` 折叠为 `/`（实测
> `Path("//server/share").resolve() == Path("/server/share")`），故**已判不构成 Linux 上的独立绕过面**；
> 但 Windows / 盘符 / 大小写不敏感文件系统 / 8.3 短名**仍未验**（见残余风险 4；`REQ-PLAT-01` 的
> 多平台 job 未建）；
> ② **"别处自建路径判定"未覆盖**（见残余风险 1）——既有机器检查只覆盖"不得在别处**定义**同名函数"
> 与"`raise PathNotAllowedError` 只应出现在 `foundation/paths.py`"；**`os.path.join` + 前缀比较在别处
> 出现不会被拦** ⇒ `R4` 的"唯一入口"是"**兑现大半、未完全兑现**"；
> ③ **配置期 / 装配期调用点不进审计**——**按 `Q-1`（2026-09-20）属「范围外」**（fail-closed、无审计；
> 规范见 [`../interfaces/audit.md`](../interfaces/audit.md) §2.6）⇒ 它是**已裁决的范围边界**，
> **不是**待补的缺口；但读者**不得**因此以为"所有路径拒绝都会留痕"。
>
> ⚠️ **本条证明了什么 / 不证明什么**（必须连读，防"已缓解并验证"被读成"安全已到位"）：
> **证明**的是"**会话内工具层的路径穿越被拒、且该拒绝留痕可回放**"这一条缓解**有可执行、可回归、
> 带变异探针的证据**（符合 `README.md` §4.1 的状态定义）；**不证明**：宿主整体安全
> （13 条里**仍有 3 条未缓解**：`T-03`/`T-04`/`T-10`）、**不证明**上面三条残余风险已消除、
> **不证明**配置期 / 装配期两类不可信输入已被处置（那两处只是**拒绝启动**、**无审计**）。
...
- **现有缓解**（**已实现**）：
  ...
  - **配置面落点白名单（2026-09-19 新增；实现与用例均已落地）**：`../interfaces/audit.md` §2.5 的
    ...
    **`W3`（根内符号链接指向根外）已于 `9b37e99` 补齐**（配置期 + 装配期 + **变异探针**：只做词法判定
    ⇒ 根内软链被误判"根内"、攻击得手）⇒ **`W1`~`W8` 全部落地**（此处更正"仍缺 `W3`"的过期表述）；
    **UNC（第 5 类）仍缺显式用例**，但已判**不构成独立绕过面**（Linux 下 `//` 折叠，见残余风险 4）。
...
- **验证方式**：
  ...
  - **范围已定案（`Q-1`，2026-09-20 所有者裁决）**：`resolve_within` 的**非工具层调用点**
    （配置期 / 装配期）在拒绝时**不进审计** ⇒ 属「**范围外**」（fail-closed、无审计；
    规范见 [`../interfaces/audit.md`](../interfaces/audit.md) §2.6；选项与代价原文见
    `README.md` §8.2 的 `Q-1`）。被排除的**只有**这两个调用点——**会话内工具层**的拒绝
    **必须**留痕，证据即上一行的两处用例。
  ...
  - **配置面落点的 `W1`~`W8`（`../interfaces/audit.md` §2.5）——2026-09-19 复核后的现状**：
    ...
    **"拒绝时不回退默认目录"** / W7 变异探针（把 `resolve_within` 换成直通实现 ⇒ W1/W5 失效）/
    W8 两层各撤一层各有用例失败；
    **`W3`（根内符号链接指向根外）已补齐**（`9b37e99`：配置期 ⇒ `ConfigError` + 装配期 ⇒
    `PathNotAllowedError` 且不向根外落盘 + **变异探针**：只做词法判定则拦截失效）
    ⇒ **`W1`~`W8` 全部落地**（"仍缺 `W3`"的旧表述已过期，本处更正）；
    **UNC（第 5 类）仍缺显式用例**，已判**不构成独立绕过面**（Linux `//` 折叠，见残余风险 4）。
...
- **相关**：`REQ-SEC-05`/`REQ-SEC-06`/`REQ-SEC-09`、`REQ-OBS-01`（审计落点可检索）、
  ...
  `../interfaces/audit.md` §2.5（配置面落点白名单）、§2.6（**审计覆盖范围**——`Q-1` 裁决：
  会话内 vs 配置期 / 装配期）。
