...
> **2026-09-19 增补（配置面落点）**：新增"**不可信配置决定审计落点**"这一类攻击面与其缓解
> **规格**（`../interfaces/audit.md` §2.5，判据 `W1`~`W8`）。**用例尚未写** ⇒ 本次
> **不升降状态、不改变 §4.1 分布与 §5 计数**（新增的是规格与待写用例，不是已落地的证据）。
...
- **现有缓解**（**已实现**）：
  ...
  - **配置面落点白名单（2026-09-19 新增，规格已定、用例待写）**：`../interfaces/audit.md` §2.5 的
    ...
    **先校验后 `mkdir`**；**禁止**越界后回退默认目录或静默关闭审计。落地位置：
    `foundation/config.py` 与 `observability/audit.py`——**均未入库、用例未写**
    ⇒ 本项当前**只是规格**，**不得**按"已缓解"记；本条只登记威胁与验证规格。
- **残余风险**：
...
- **验证方式**：
  ...
  - **仍缺（行为，2026-09-19 新增）**：配置面落点的 `W1`~`W8`（`../interfaces/audit.md` §2.5）——
    根外目录 / `..` 上跳 / 根内符号链接指向根外 / 绕过配置直接构造 sink / **"拒绝时不创建目录"** /
    **"拒绝时不回退默认目录"** / 变异探针（去掉 `resolve_within` 后至少 3 条失败）/
    两层各删一层各有用例失败。落点 `tests/security/`（**由验证角色写**）。
