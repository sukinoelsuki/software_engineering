...
| **A-6** | **审计记录**（`AuditEvent` 流） | `REQ-SEC-06` 的"可回放"是安全需求的验收对象 | **高**（事件丢失 = 验收失败；且事后无法补录） | 已实现（`observability/audit.py`，`e78c221`） |
...
| **A-10** | 多会话 / 长任务的**上下文与检查点**（harness 会话状态） | 被污染后跨会话持续影响（记忆投毒） | **中**（可丢弃会话，但污染若已落盘则难察） | 已实现（`harness/checkpoint.py`，`cbef66f`） |
| **A-11** | **平台写权限**（`CNB_TOKEN` **与其执行者 `cnb-cli`**） | 它能改远端状态（分支 / PR / Issue / 流水线 / 制品） | **中**（A 类可 `git revert`；凭据泄漏、改写已推送历史、`main`、`bench/data` **不可撤**——判据见 `ADR-0016` §5.2） | 环境变量（令牌，**已在用**）+ 镜像内 CLI（**尚未接入**） |

> **`A-11` 的接入状态（2026-09-18 实测，不得当作"已生效"）**：`CNB_TOKEN` **已经在被使用**
> ——开发环境环境变量与基准流水线各一处（`ADR-0016` §1.2）；
> 而 `cnb-cli` 与官方 CNB Skill **尚未接入**：实测 `command -v cnb` 无输出、
> `~/.codebuddy/skills/` **不存在**。⇒ 本条把"**权限**"与"它的**执行者（CLI）**"合成一类资产，
> **不是**声称 CLI 已可用；接入状态以 `.ide/Dockerfile` 与构建期自检为准（`ADR-0016` §5.3/§7）。
...
>
> **`A-11` 是本轮新增**（依据 `ADR-0016` §5.7）：它被 `T-08`（凭据泄漏）、
> `T-09`（CLI 作为新增的第三方代码与**指令来源**）、`T-11`（特权工具的越权使用）三条指向。
> 它**不是一类新权限**——令牌早已存在——而是把"**早已存在的平台写权限**"从 `A-5`
> （作为**秘密**的凭据）里**单列**出来，因为两者的失效模式不同：
