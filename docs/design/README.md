# 设计目录

本目录存放**架构设计、模块设计、接口契约与威胁模型**。

---

## 计划结构

```text
design/
├── architecture.md          总体架构（分层、组件、数据流、部署形态）
├── modules/                 各模块详细设计
│   └── <module>.md
├── interfaces/             接口契约（类型、错误语义、并发假设）
│   └── <interface>.md
├── diagrams/               架构图、时序图（源码形式，如 Mermaid / PlantUML）
├── threat-model/           威胁模型（见下）
│   ├── README.md           怎么用 + 方法 + 范围 + 条目总览 + 缺验证/未缓解清单
│   ├── assets.md           资产与信任边界
│   ├── execution-and-isolation.md     条目组 A：T-01 / T-02 / T-10 / T-13
│   ├── untrusted-input-and-agentic.md 条目组 B：T-03 / T-04 / T-05 / T-11 / T-12 / T-15
│   └── supply-chain-and-process.md    条目组 C：T-06 / T-07 / T-08 / T-09 / T-14
└── security-model.md       权限/能力模型设计
```

> 上述结构将在 Phase 1 建立。**新增目录需先在 ADR 中说明理由。**
>
> **建立进度（2026-09-18）**：
>
> - `interfaces/` **已建立** —— 字段级契约见 [`interfaces/`](interfaces/README.md)
>   （为 `contracts/` 的 12 个类型定死字段；依据 ADR-0015 §5.1.2 与 §7.3）。
> - `threat-model/` **已建立（初稿）** —— 见 [`threat-model/`](threat-model/README.md)：
>   **13 条**威胁（`T-01`~`T-13`），含状态分布、缺验证清单与未缓解项清单。
>   **条目文件的组织方式与本节原计划不同**：原计划"每条威胁一份 `T-<序号>.md`"，
>   实际按**域分 3 个文件**——理由：13 个碎片文件不便通读（本项目要求威胁模型
>   **给人读**，见 `SECURITY.md` §4），且同域条目共享同一组前提与缓解，
>   合并后**重复叙述更少**；`T-<序号>` 仍作为**跨文件稳定的引用键**保留
>   （编号只增不复用，规则见该目录 `README.md` §1）。
> - `architecture.md` **已建立**（2026-09-19，成稿）—— 见 [`architecture.md`](architecture.md)：
>   分层与依赖方向（`R1`~`R5` 与机器检查逐条对应）、组件与自研边界（含"不写什么"清单）、
>   关键路径的数据结构与算法（含 `default-deny` 的拒绝路径）、并发与资源生命周期、部署形态，
>   并含 **「模块 × 当前状态（已实现 / 未开工）× 依据（提交哈希或文件名）」** 表。
>   图一律 **Mermaid 内联**在本文件内（`diagrams/` 未建立——新建目录须先在 ADR 中说明理由）。
> - 其余（`modules/` / `security-model.md`）**尚未建立**。

---

## 设计要求

1. **先接口后实现**：模块间接口（类型、错误语义、并发假设、资源生命周期）必须先定义。
2. **图用文本形式**（Mermaid 优先），保证可 diff、可评审、可版本管理。
3. **设计必须可实现**：避免只画方块的"PPT 架构"；关键路径要说明数据结构与算法。
4. **设计与实现保持一致**：实现偏离设计时，同步更新设计文档（属于 DoD 的一部分）。

---

## 威胁模型要求

威胁模型是**前置交付物**，与设计同步产出，不是事后补写。

### 方法

- 基线：**STRIDE**（欺骗、篡改、抵赖、信息泄漏、拒绝服务、权限提升）
- 智能体特有风险：结合 **OWASP Agentic AI Top 10**、**MITRE ATT&CK for MCP**
  等框架，单列以下类别：
  - 提示注入（直接 / 间接）、指令与数据混淆
  - 工具滥用与越权调用
  - 上下文污染与记忆投毒
  - 供应链投毒（工具描述、插件、模型权重、依赖）
  - 沙箱逃逸与资源耗尽

### 每条威胁必须包含

```text
T-<序号>：<威胁名称>
- 资产：被保护的对象
- 攻击面：攻击者可达的入口
- 攻击路径：具体步骤（可复现）
- 影响：机密性 / 完整性 / 可用性 / 合规
- 缓解措施：设计层面的对策
- 验证方式：**可执行的**测试用例或对抗性输入集
- 残余风险：缓解后仍存在的风险，以及是否可接受
```

**没有"验证方式"的缓解措施视为未实现**——它是纸面防护，无法证明有效。

> **已落地（2026-09-18）**：上述要求已落成
> [`threat-model/`](threat-model/README.md)（初稿 13 条；**当前 15 条**——
> `T-14` 于 2026-09-25 新增、`T-15` 于 2026-09-25 新增，**条数与状态的唯一真源是该目录 `README.md` §4/§4.1**）。实际条目在必填字段之外
> **另加三个字段**，把"纸面防护"与"已生效防护"区分开：
> **`状态`**（已缓解并验证 / 部分缓解 / 未缓解，定义见该目录 `README.md` §4.1）、
> **`攻击者与前提`**（谁能做什么才有机会——没有前提的攻击路径是幻想）、
> **`缺验证`**（该条目当前**没有**对应用例这一事实本身）。
> ⇒ 读威胁模型时**先看状态列**：本项目当前是 **1 条"已缓解并验证"**（`T-02`，2026-09-20 所有者裁决 `P-2`；依据见 `threat-model/README.md` §4.1）——⚠️ **但"1"本身不构成结论**：它是"**待办清单式**"盘点的当前值（**未缓解仍有 `T-04`/`T-10`**，各条缺口照旧），**不得**读成"安全已到位"。
