...
**已缓解并验证 1 条 / 部分缓解 10 条 / 未缓解 2 条**（`threat-model/README.md` §4.1），
口径是"**待办清单式的威胁模型**"。`ADR-0015` §7.2 的 `S1`（未授权工具调用被拒**且**审计可回放）
与 `S3`（领域包内 `.py` **永不**被导入）**已落地**，`S2` 的"拒绝"半与"且留审计"半（工具层）
**均已落地**。`T-02` 已于 **2026-09-20** 由所有者裁决升为「**已缓解并验证**」（项目首次），
其残余风险见威胁模型条目；配置期 / 装配期调用点按 `Q-1` 裁决为**范围外**（fail-closed、
无审计、理由写明）。
...
| `docs/design/threat-model/`（4 份） | 威胁模型 | **已建立（初稿）**：13 条，**1 已缓解并验证 / 10 部分缓解 / 2 未缓解**（⚠️ 本行**此前遗漏同步**：上一轮 2026-09-20 状态同步未覆盖本行，本行当时仍停留 `0 / 8 / 5`，本次一并更正——做**更正**，不抹掉原值） | `threat-model/README.md` §4.1 |
...
| `tests/unit/` | 单测 | **已建立**（**42** 个 `test_*.py` 模块；**不含** `conftest.py` 等非 `test_*.py` 文件 ⇒ 目录下共 43 个 `.py`） | `ls tests/unit/test_*.py \| wc -l` = `42`（2026-09-20 实测；与 [`testing-strategy.md`](../engineering/testing-strategy.md) §2 **同口径、同命令**） |
| `tests/security/` | 对抗性用例（含 `corpus/`） | **已建立**（**18** 个 `test_*.py`；另有 `__init__.py` / `conftest.py` / `_harness_fakes.py` / `corpus/`，**均不计入模块数**；`corpus/` 目前**只有路径穿越语料，无注入语料**） | `ls tests/security/test_*.py \| wc -l` = `18`（2026-09-20 实测，同口径同命令）；`threat-model/README.md` §0 |
| `tests/integration/` | 端到端 1 条（可标记 `slow`） | **已建立**（1 条真模型端到端；标 `slow`、默认不跑） | `tests/integration/test_end_to_end.py`（`fba8c4c` / `b1fc35a`） |
...
> **本节的可信度声明**：§5.1 的各段**均有实现**，且**串接（`Session` 装配）已由 `cli/app.py` 落地**
> （`4baa9e8`）；§5.2 的时序图仍按"规划态"逐段标注实现状态——**串接存在 ≠ 端到端已跑通**
> （是否跑通以 `tests/integration/` 与 `docs/devlog/` 为准）。读本节时请把"规划"与"已实现"分开看。

### 5.1 装配阶段（会话建立之前，各段**已实现**，串接点由 `cli/app.py` 落地）
...
flowchart TD
    ...
    RESOLVE -- "None：模型幻觉的工具" --> DENY0["默认拒绝 + 审计<br/>【HARNESS 侧，已实现】"]
    RESOLVE -- "Tool" --> VALID{"按 parameters_schema 严格校验并解析原始 JSON"}
    VALID -- "不合法" --> DENY2["不构造 PolicyRequest；以 ToolResult(ok=False) 回喂<br/>【HARNESS 侧，已实现】"]
...
| 威胁 | 落到哪个模块 / 文件 | 当前状态（2026-09-20，以威胁模型为准） |
| --- | --- | --- |
| `T-01` 子进程执行与逃逸 | `foundation/proc.py`、`tools/shell.py`、`security/sandbox/`（未开工） | 部分缓解 |
| `T-02` 路径穿越 | `foundation/paths.py`、`tools/registry.resolve_tool_path`、`observability/audit.py`（落点白名单） | **已缓解并验证**（2026-09-20 所有者裁决 `P-2`；残余风险：`UNC` 跨平台未验 / "别处自建路径判定"未覆盖 / 配置期·装配期调用点不进审计＝`Q-1` 判为范围外） |
| `T-03` 指令-数据混淆 | `harness/`（已实现）、`contracts/model.py`（信任规则） | **部分缓解**（2026-09-20 所有者裁决 `Q-3`；⚠️ 缺口**照旧**：三条核心攻击路径 (a) 观察文本改控制流 / (b) `arguments` 拼接 / (c) 工具内二次解析**均无专例**，**注入语料集仍不存在**——见 `threat-model/README.md` §4.1 的 `Q-3` 段） |
| `T-04` 提示注入与上下文污染 | `harness/{context,prompts}`（已实现）、`tests/security/corpus/`（无注入语料） | **未缓解** |
...
| `T-11` 越权工具调用与工具滥用 | `security/policy.py`、`tools/registry.py`、`harness/`（已实现） | **部分缓解** |
| `T-12` 领域包加载代码 | `harness/domain_pack`（已实现） | **部分缓解**（静态守卫 + `S3` 行为用例已落地；缺口：`.pth`/`.so` 等向量未断言、仅覆盖领域包这一载体） |
| `T-13` 隔离机制静默失效 | `security/sandbox/`（未开工）、`foundation/proc.py` | 部分缓解 |

> **状态分布合计：已缓解并验证 1 / 部分缓解 10 / 未缓解 2。**
...
| `G-2` | **`D2`（pydantic 的两处用途）无载体，且 `parameters_schema` 是手写 dict** | **设计 ↔ 实现口径差异** | 撰写时的事实（**不变**）：`src/` 中无 `pydantic` 的 import（唯一第三方运行期 import 是 `foundation/logging.py` 的 `structlog`）；内置工具的 `parameters_schema` 由**手写 dict** 给出（`tools/files.py`、`tools/shell.py`），而 `ADR-0015` §5.2.2 的落法是"由 pydantic 模型类生成" | **已处置（2026-09-19，`ADR-0020` 已接受）**：新增 [`ADR-0020`](../adr/0020-argument-validator-implementation.md) —— ① 信任边界校验改用**手写 JSON-Schema 子集校验器**（`harness/arguments.py`，已落地 `43a177c`）；② `parameters_schema` **保留手写 dict**；两者合起来登记为"**`D2` 两处用途均不落地**"（`ADR-0015` 正文不改，只追加修订记录指针）。✅ `ADR-0020` 已于 **2026-09-19 获所有者批准（已接受）** ⇒ 本行按**已处置**理解；⚠️ `pyproject.toml` 的 `pydantic` 去留**仍未决**，属 **`F` 类**，见该 ADR §8 动作 6 |
...
| `G-5` | 审计落点的两项**策略待确认项** `A1` / `A2` | **已拍板（2026-09-20，所有者）** | `A1`：是否允许 `ALLOWED_AUDIT_ROOTS` 含额外根（CI 挂载卷 / 演示归档）；`A2`：是否移除 `audit.directory` 键（只留 `filename`）。**裁决：两项均维持最保守取值**——`A1` 取 ①（单一根，**不含**额外根）、`A2` 取 ①（**保留** `audit.directory` 键，校验已收窄为"根内路径"）⇒ 两项**不再是待确认项**，实现与单测**零改动** | 见 `interfaces/audit.md` §5（已登记裁决）。**注意**：无论哪一项，`P1`~`P7` 与 `W1`~`W8` **不随之变化** |
| `G-6` | **端到端闭环无用例（原测试缺口）** | **已解除** | `ADR-0015` §5.4.2 要求 B 阶段落 1 条端到端（启动 `llama-server` → 读文件 → 调模型 → 执行工具 → 回喂），可标记 `slow` | **已落**（`tests/integration/test_end_to_end.py`，`fba8c4c` / `b1fc35a`；标 `slow`、默认不跑）；**是否跑通以实跑记录为准**（`docs/devlog/`）。`sdlc.md` §6 的"集成"工件已具备 |
| `G-7` | `S1` / `S3` / `S2` 的"审计"半（原缺口登记） | **已解除** | 原判（保留不改）：被测实现（`harness/`、审批门、`domain_pack` 加载器）不存在 ⇒ 验证者**拒绝造测**（正确处置，口径见 `SECURITY.md` §4） | **已解除**（2026-09-20 同步）：`S1`/`S3`/`S2` 的**工具层审计半均已落地**（`tests/security/test_harness_s1_authorization.py`、`test_s1_replayable_audit_link.py`、`test_harness_s3_domain_pack.py`，2026-09-19 入库）；**非工具层调用点按 `Q-1` 裁决为范围外**。历史口径保留：**落地前不得视为已覆盖**（当时因被测实现不存在而如此判） |
| `G-8` | 契约层无法强制的两条前置条件 | **已登记的部分缓解** | `PolicyRequest.requested` 的"非空"与"元素必须是 `Capability` 实例"在**类型层面表达不了**（`frozenset` 表达不了非空；`contracts/` 零行为不能加 `__post_init__`） | 兜底由 `PolicyEngine` 的防御分支负责（**已实现**：`1a`/`1b`）；上游由 `harness/` 构造点保证（**已实现**）。按威胁模型口径记为**部分缓解** |
| `G-9` | **示例领域包（`examples/packs/`）的定位与分发口径** | **新目录（本 ADR 说明理由）+ 分发口径差异（待验证）** | **定位**：`examples/packs/<name>/` 是**声明式数据**（`pack.toml`），作为**产品正规通路**给出"非交互 + 未配 pack ⇒ 零工具可执行"这一**可用性缺口**（**不是安全缺陷**）的最短可用路径；**被 `load_pack` 读盘、不参与导入、不属于 `src/` 分层**，且**按不可信输入对待、无旁路**。决策与边界见 [`ADR-0021`](../adr/0021-example-domain-packs-and-first-usable-release.md)（**提议中**）。**现状**：本批由实现工程师落地**只读**示例包（`42f36a3`：`examples/packs/coding-readonly`、`examples/packs/tech-manual-qna`，各只声明 `read_file` / `list_dir` 且仅将其声明为 `low`；`examples/README.md` 给用法），并有机器检查 `tests/unit/test_example_packs.py` 钉住"只读边界"；**是否可用 / 已跑通一次端到端以验证角色的实跑证据为准——本文不宣称已跑通**。⚠️ **零安全默认改动**：`DEFAULT_TOOL_RISK` 保持 `HIGH`、`PolicyEngine` 不改（`ADR-0021` §5.1 的 `B-1`） | **待验证**：当前构建配置 `[tool.hatch.build.targets.wheel] packages = ["src/agent_sec_perf"]` **不含** `examples/` ⇒ **预期不随 wheel 分发**（`uv build` 后 `unzip -l dist/*.whl \| grep -c examples/` 预期 `0`）；若需随 wheel 分发，须改构建配置（属 **`F` 类**，另开决策）。另：`src/` 不得 import `examples/` 这一不变性**暂无机器检查**（`ADR-0021` §7 第 4 条，**待落地**） |
...
| 2026-09-19 | **现状同步（`harness/` 相关实现状态 + `tests/integration` + `G-2`/`G-6`/`G-8`）**：① §4.4 的 `tests/integration/` 行由"未建立"改为**已建立**（`tests/integration/test_end_to_end.py`，`fba8c4c`），`G-6` 由"测试缺口"改为**已解除**；② §5 的可信度声明与 §5.1 标题：串接（`Session` 装配）**已由 `cli/app.py` 落地**（`4baa9e8`），但**"串接存在 ≠ 端到端已跑通"**；③ §5.3 的拒绝路径 flowchart 两个 `【HARNESS 侧，未开工】` 节点改为**已实现**；④ §7.2 威胁落位表的**模块列**：`T-03`/`T-04`/`T-11`/`T-12` 的 `harness/`·`harness/domain_pack` 标注由"未开工"改为**已实现**（`cb3157e` / `a9f73ee` / `14a7831`）；⑤ `G-2` 由"待批准"改为**已接受**（`ADR-0020` 已获批准、`harness/arguments.py` 已落地）；`G-8` 的上游 `harness/` 构造点由"未开工"改为**已实现**。⚠️ **本笔只改实现状态**：§7.2 的**状态列**（`T-XX` 部分缓解 / 未缓解）、§7.2 的状态分布合计（0 / 8 / 5）、`T-11` 的"（`S1` 未落地）"与 `T-12` 的"（`S3` 未落地）"、`G-7`、以及 §0 第 2 条的 `S1`/`S3` 表述**一律未动**（涉 `T-XX` 结论与计数，按领导 2026-09-19 指示**只出提案**） | 读源码与提交核实（`git log --oneline -- tests/integration src/agent_sec_perf/cli src/agent_sec_perf/harness`；`ls tests/integration`）；`threat-model/README.md` §7 与 `interfaces/harness.md` 的对应记录 |
| 2026-09-20 | **状态同步（威胁模型裁决联动）**：§0 第 2 条的状态分布 `0 / 8 / 5` → `1 / 9 / 3`，`S1`/`S3` 由"仍未落地"改为**已落地**、`S2` 的"拒绝"半与"且留审计"半（工具层）**均已落地**，并补记 `T-02` 已于 2026-09-20 由所有者裁决升为「**已缓解并验证**」（项目首次）、配置期 / 装配期调用点按 `Q-1` 裁决为**范围外**；§7.2 落位表的 `T-02` 行状态列 → **已缓解并验证**（标注 `P-2`）、`T-11`/`T-12` 行 → **部分缓解**（删去"`S1`/`S3` 未落地"）、合计行 `0 / 8 / 5` → `1 / 9 / 3`；§11 的 `G-7` 由"安全覆盖缺口"改为**已解除**（保留原判据与"落地前不得视为已覆盖"的历史口径）。**不改任何决策**：§2.1 的 `R1`~`R5`、§2.3 的依赖白名单、§3 的组件选型、§4 的实现状态表、§5 的算法与路径一律不变（**"已实现" ≠ "已缓解"**） | 所有者 2026-09-20 裁决：`Q-1`（`resolve_within` 非工具层调用点拒绝不进审计＝**范围外**，fail-closed、无审计、理由写明）与 `P-2`（`T-02` 升为**已缓解并验证**，项目首次）；依据用例：`tests/security/` 的 `test_harness_s1_authorization.py`、`test_s1_replayable_audit_link.py`、`test_harness_s3_domain_pack.py`（均 2026-09-19 入库）；`docs/design/threat-model/README.md` §8.2 |
| 2026-09-20 | **状态同步（威胁模型 `Q-3` 裁决联动）**：① §0 第 2 条的状态分布 `1 / 9 / 3` → `1 / 10 / 2`；② §7.2 落位表的 `T-03` 行状态列 → **部分缓解**（标注所有者裁决 `Q-3`，并**保留其缺口**：三条核心攻击路径 (a)(b)(c) 与注入语料集仍缺）；③ §7.2 的合计行 `1 / 9 / 3` → `1 / 10 / 2`；④ **另更正一处此前遗漏的同步**——§4.4 的威胁模型行仍停留 `0 / 8 / 5`（上一轮 2026-09-20 状态同步**未覆盖本行**），本次一并更正为 `1 / 10 / 2`（**登记遗漏事实，不静默改口**）。**不改任何决策**：§2.1 的 `R1`~`R5`、§2.3 的依赖白名单、§3 的组件选型、§4 的实现状态表、§5 的算法与路径一律不变（**"已实现" ≠ "已缓解"**；本文件**不得**被读成"安全已到位"，§0 第 2 条） | 所有者 2026-09-20 裁决 `Q-3`（`T-03` 由「未缓解」升「**部分缓解**」；`T-04`/`T-10` **维持「未缓解」**，"已缓解并验证"保持 `T-02` 一条）。**唯一真源**：`docs/design/threat-model/README.md` **§4.1**（分布 `1 / 10 / 2`）与其 §8.2 的 `Q-3`【裁决后状态】块 |
| 2026-09-20 | **事实更正（§4.4 测试分层的过期计数）**：`tests/unit/` 由“22 个模块”更正为 **42 个 `test_*.py`**（目录下共 43 个 `.py`）；`tests/security/` 由“11 个模块”更正为 **18 个 `test_*.py`**（另有 `__init__.py` / `conftest.py` / `_harness_fakes.py` + `corpus/`）。**同批复核后无需改**：`tests/integration/`（1 条真模型 e2e；标 `slow`、默认不跑）、`tests/benchmark/`（不新增，基准由 `bench/` 承担）、`interfaces/`（6 份契约）、`threat-model/`（4 份）、`modules/`·`diagrams/`·`security-model.md`（均未建立）。⚠️ **只改现状计数，不改任何决策**：§2.1 的 `R1`~`R5`、§2.3 的依赖白名单、§3 的组件选型、§5 的算法与路径、§7.2 的威胁落位与状态计数（`1 / 10 / 2`）一律不变 | `ls tests/unit/`、`ls tests/security/`、`ls docs/design/interfaces/`、`ls docs/design/threat-model/` 实测（2026-09-20）；同批 [`testing-strategy.md`](../engineering/testing-strategy.md) §2 已按实测重算（`-m unit` 844〔⚠️ **该值当时即已过期 —— 见下一行更正为 `854`**〕 / `-m security` 96） |
| 2026-09-20 | **事实更正（测试分层数字 + 取证口径）**：① 上一条 §12 引用的 `-m unit` **`844` 为当时即已过期的值**，本行更正为 **`854`**（上一条其余内容不变、不改写）。② §4.4 的 `tests/unit/` / `tests/security/` 两行补上**与 [`testing-strategy.md`](../engineering/testing-strategy.md) §2 同一口径、同一命令**的取证方式（`ls <层>/test_*.py \| wc -l` ⇒ `42` / `18`，**不含** `conftest.py` / `__init__.py` / `_harness_fakes.py` / `corpus/`），**消除两处数字分叉的种子**。**只改现状计数与取证方式，不改任何决策**：§2.1 的 `R1`~`R5`、§2.3 依赖白名单、§3 组件选型、§5 算法与路径、§7.2 威胁落位与状态计数（`1 / 10 / 2`）一律不变 | `ls tests/unit/test_*.py \| wc -l` = `42`、`ls tests/security/test_*.py \| wc -l` = `18`、`uv run pytest -m unit --collect-only -q` 末行 = `854/960 tests collected`、`uv run pytest -m security --collect-only -q` 末行 = `96/960 tests collected`（2026-09-20 **原样复跑**）；同批 `testing-strategy.md` §2 |
