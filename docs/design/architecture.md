...
| `tests/unit/` | 单测 | **已建立**（**42** 个 `test_*.py` 模块；目录下共 43 个 `.py`） | `tests/unit/`（`ls tests/unit/*.py` 实测，2026-09-20） |
| `tests/security/` | 对抗性用例（含 `corpus/`） | **已建立**（**18** 个 `test_*.py` + `__init__.py` / `conftest.py` / `_harness_fakes.py` + `corpus/`，`corpus/` 目前**只有路径穿越语料，无注入语料**） | `tests/security/`（`ls tests/security/` 实测，2026-09-20）；`threat-model/README.md` §0 |
