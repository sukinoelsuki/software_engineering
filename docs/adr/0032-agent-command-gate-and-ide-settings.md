...
- **2026-09-25（修订 · `U4` 的处置：决定不专程验证）**：所有者指示**不做**"有 stage / 删 stage"的对照
  （"当前任务认为结束了"）⇒ §10 的 **`U4` 自此不再作为待办**（正文按"只增不改"保留，
  **以本条为准**）：`U4` 仍**不得**被写成"已证实/已推翻"，本决策的取舍也**不依赖它**
  （保留 COPY + 拆分常变部分已结案）。对照配方**归档保留**在
  [devlog 0026](../devlog/0026-2026-09-25-收尾承接与运行期改写观察项.md) §2.2，
  将来若因别的原因需要定主体可直接照做；处置口径与 §3 对"并发上限探针"的处理同一手法。
