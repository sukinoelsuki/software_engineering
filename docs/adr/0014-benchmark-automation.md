...
#### 2.9.2 规则文本澄清建议（**待领导处理**；本节只记录冲突与建议）
...
**建议措辞（一句话，同时表达三件事：① 标记只写规则号 ② 理由写在紧邻的独立注释行
③ 禁止在标记后追加自由文本——它会**静默放宽检查**）**：

> **确需豁免时，豁免标记所在行只允许出现规则号；禁止在 `# nosec` / `# noqa` 之后追加任何自由文本
> （可能被工具当作规则号而静默放宽检查）；理由必须写成紧邻该标记的独立注释行。**

**属规则变更**，需按项目"规则变更的联动更新"表同步（下表只列建议；
**规则文件不在架构师域，由领导处理**）：

| # | 位置 | 需改什么 | 谁改 |
| --- | --- | --- | --- |
| 1 | `.codebuddy/rules/security-baseline` | "在该行写明理由" → 用上面的**一句话**替换；补"标记行只写规则号" | **领导** |
| 2 | `SECURITY.md` | 与规则文件**逐字一致**地同步同一句话 | **领导** |
| 3 | `pyproject.toml` 的 `[tool.bandit]` 注释 | 现注释称豁免需"在该行标注 `# nosec <理由>` 并登记到 SECURITY.md"；`0c657d3` 已统一过一次，需再对齐"登记位置"与"标记行只写 ID"两句 | **领导** |
| 4 | `docs/engineering/definition-of-done.md` 第 17 行 | "无新增的 `# noqa` / `# nosec` / `# type: ignore`，**或已写明理由并登记**"——"写明理由"须按 F1 澄清为"**在紧邻的独立注释行写明**"，并与上面的**一句话**逐字一致 | **领导** |
| 5 | 本 ADR §2.9.1(b) | 已是实测结论与统一约定的载体，无需再改 | 架构师 |

> **落地后必须做反向搜索**：以旧措辞（"在该行写明理由"、`# nosec <理由>`）在 `.codebuddy/`、
> `SECURITY.md`、`pyproject.toml`、`docs/engineering/` 内搜索，确认无残留旧表述
> （项目"规则变更的联动更新"一节明确要求）。
