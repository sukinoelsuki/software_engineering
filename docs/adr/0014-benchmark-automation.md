...
**关于行号（此结论仍然成立）**：上表与本节的 `L23 / L145 / L187 / L223` 是 **2026-09-18 的快照**；
...
> 确需豁免时必须在**豁免标记所在行或紧邻该行**写明理由；
> **豁免标记所在行只允许出现规则号**，不得在标记之后追加任何文本；
> 理由应写成紧邻的独立注释行，且理由文本中**不得出现其它规则号**
> （防止被工具误读为额外的豁免项）。
...
| # | 位置 | 需改什么 | 谁改 |
| --- | --- | --- | --- |
| 1 | `.codebuddy/rules/security-baseline` | "在该行写明理由" → "**在该行或紧邻该行**"；补"标记行只写规则号" | **领导** |
| 2 | `SECURITY.md` | 与规则文件**逐字一致**地同步同两句 | **领导** |
| 3 | `pyproject.toml` 的 `[tool.bandit]` 注释 | 现注释称豁免需"在该行标注 `# nosec <理由>` 并登记到 SECURITY.md"；`0c657d3` 已统一过一次，需再对齐"登记位置"与"标记行只写 ID"两句 | **领导** |
| 4 | 本 ADR §2.9.1(b) | 已是实测结论与统一约定的载体，无需再改 | 架构师 |
