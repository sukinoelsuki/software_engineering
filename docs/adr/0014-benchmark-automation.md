...
**采纳状态**：**已采纳并落地**（`187c610` / `a8d78b4`），保留了本建议的核心
（标记行只写规则号 + "理由中不得出现其它规则号"）；但有**一处用词差异**——
规则文本写作"理由写在豁免标记**所在行**或紧邻其上的独立注释行"，比本建议**多出"所在行"**。

> **注意：该差异与同一条的 ① 互相张力（建议下次改规则时消除）**。同一条 ① 明确
> "**不得在标记之后追加任何文本**" ⇒ 标记行上根本无处写理由，**"或所在行"是不可达分支**；
> 但它的存在会让下一位读者以为"写在标记行也行"，从而**重新引入本节的失效模式**。
> **建议**：删去"或豁免标记所在行"，或明确其含义（若确指多行语句的"语句行"，需与 ① 一并改写）。
> **澄清前按更严的一方执行**：理由只写**紧邻的独立注释行**。
...
| 4 | `docs/engineering/definition-of-done.md` 第 17 行 | 仍为原文「无新增的 `# noqa` / `# nosec` / `# type: ignore`，**或已写明理由并登记**」——**不含** F1 的具体指引。它**不带"在该行"字样**，故**不与 F1 冲突**；但也**未约束**标记行的写法 | **待领导确认**（见下"核对差异"） |
| 5 | 本 ADR §2.9.1(b) | 已是实测结论与统一约定的载体 | 无需再改（架构师） |

> **核对差异（如实记录，不采信转述）**：领导消息称"1/2/3/4 全部完成"，但 2026-09-18 我**逐字读取**
> `docs/engineering/definition-of-done.md` 第 17 行，仍是原文、未见 F1 指引。两种可能：
> ① **漏改**；② 判断该行"不带'在该行'字样、与 F1 不冲突"故**无需改**。
> **若是 ②，建议至少加一句指向 `S-7`**——否则 DoD 的"已写明理由"与 `S-7` 的"两步写法"
> 构成**两套口径**，正是本项目反复吃亏的漂移型态。
