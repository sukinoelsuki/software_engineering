...
- **状态**：**已接受**（所有者 2026-09-20 批准）
- **落地与批准的同批关系（如实登记，**保留**）**：`examples/packs/` 的目录与只读示例包
  **已由实现工程师在同批落地**（提交 `42f36a3`，2026-09-20），**早于**本 ADR 状态改为「已接受」
  ——这是一处**刻意的顺序偏差**（先落地、后走批准流程），在此**如实登记、不作瑕疵掩盖**。
  它**不构成**任何授权外推：`examples/` 只含声明式数据，`§5.1` 的 7 条硬约束（零安全默认改动等）一并不变。
- **日期**：2026-09-20
...
- [x] **动作 1（2026-09-20 完成）**：本 ADR 获批后把「状态」改为「已接受」并在
  [`docs/adr/README.md`](README.md) 索引登记（**正文不改**）。
  **完成日期与依据：2026-09-20，所有者批准**（授权原话「批准方案」）；状态行与本文件 `§9`、
  `docs/adr/README.md` 索引已同批同步。
...
| 2026-09-20 | **状态变更（只追加；不改正文结论）**：所有者批准 ⇒ 状态由「提议中」改为「**已接受**」；§8 动作 1 打勾（含 `docs/adr/README.md` 索引同步）。**正文论证与 §5.1/§5.2 的结论一律未改**（ADR 只增不改：历史 ADR 的结论保持不变） | 所有者 2026-09-20 批准（授权原话「批准方案」）；`docs/adr/README.md` 索引同步（本次提交） |
| 2026-09-20 | **事实登记（只追加；正文不改）**：§5.2 正文第 199 行写的「（当前 `[project].version = "0.0.1"`；`0.1.0` 是台账中**尚未到达**的下一个里程碑。）」是**撰写当日的快照**——当日 `0.1.0` 已由 `make release` 落定（提交 `a8a30e8`），且台账已补 `0.2.0`（提交 `5648bde`）⇒ **该句作为历史快照保留**，读时以 `pyproject.toml` 的 `[tool.lowspec.releases]` 台账为准 | 提交 `a8a30e8`（`chore(release): 0.1.0`，2026-09-20 19:07）、`5648bde`（`docs(repo): 落地 ADR-0021 批准、M1 出口准则生效与 M2 准则及 0.2.0 台账`，2026-09-20 19:38）；`pyproject.toml` 台账原文（**读文件核实**，非推测） |
